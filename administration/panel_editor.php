...
if (isset($_POST['save'])) {
	...
	if (isset($_GET['panel_id']) && isnum($_GET['panel_id'])) {
		if ($panel_name) {
			...
			if ($panel_name != $data['panel_name']) {
				if (dbcount("(panel_id)", DB_PANELS, "panel_name='".$panel_name."'") > 0) { $error .= $locale['471']."<br />"; }
			}
		}
		...
		if (!$error) {
			log_admin_action("admin-3", "admin_panel_updated"); // Log Admin's Action
		...
		if (!$error) {
			log_admin_action("admin-3", "admin_panel_saved"); // Log Admin's Action
	...
	if (isset($_POST['preview'])) {
		log_admin_action("admin-3", "admin_panel_previewed"); // Log Admin's Action
		$panel_name = stripinput($_POST['panel_name']);
	...
	if (isset($_GET['panel_id']) && isnum($_GET['panel_id'])) {
		...
		if (!isset($_POST['preview'])) {
			$panel_name = "";
			$panel_filename = isset($_GET['filename']) && !empty($_GET['filename']) ? $_GET['filename'] : ""; // Pimped
			$panel_content = "openside(\"name\");\n"."  echo \"content\";\n"."closeside();";
			$panel_type = isset($_GET['filename']) && !empty($_GET['filename']) ? "file" : ""; // Pimped
			$panel_side = "";
			$panel_access = "";
			$panelon = "";
	...
	if ((isset($_GET['panel_id']) && isnum($_GET['panel_id'])) || (isset($_GET['filename']) && !empty($_GET['filename']))) { // Pimped
		if ($panel_type == "php") {
	...
	echo "</tr>\n<tr>\n";
	echo "<tr>\n<td class='tbl'>";
	echo info_helper("panel_editor", true); // Pimped
	echo "</td>\n";
	echo "<td class='tbl'>\n";
