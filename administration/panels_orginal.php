...
if (isset($_GET['action']) && $_GET['action'] == "refresh") {
	...
	while ($data = dbarray($result)) {
		$result2 = dbquery("UPDATE ".DB_PANELS." SET panel_order='".(int)$i."' WHERE panel_id='".(int)$data['panel_id']."'");
		$i++;
	}
	$i = 1;
...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['panel_id']) && isnum($_GET['panel_id']))) {
	$data = dbarray(dbquery("SELECT panel_order, panel_side FROM ".DB_PANELS." WHERE panel_id='".(int)$_GET['panel_id']."'"));
	$result = dbquery("DELETE FROM ".DB_PANELS." WHERE panel_id='".(int)$_GET['panel_id']."'");
	$result = dbquery("UPDATE ".DB_PANELS." SET panel_order=panel_order-1
	WHERE panel_side='".(int)$data['panel_side']."' AND panel_order>='".(int)$data['panel_order']."'");
	log_admin_action("admin-3", "admin_panel_deleted"); // Log Admin's Action
	redirect(FUSION_SELF.$aidlink);
}
if ((isset($_GET['action']) && $_GET['action'] == "setstatus") && (isset($_GET['panel_id']) && isnum($_GET['panel_id']))) {
	log_admin_action("admin-3", "admin_panel_status".$_GET['status']); // Log Admin's Action
	$result = dbquery("UPDATE ".DB_PANELS." SET panel_status='".intval($_GET['status'])."' WHERE panel_id='".(int)$_GET['panel_id']."'");
}
if ((isset($_GET['action']) && $_GET['action'] == "mup") && (isset($_GET['panel_id']) && isnum($_GET['panel_id']))) {
	...
	$result = dbquery("UPDATE ".DB_PANELS." SET panel_order=panel_order+1 WHERE panel_id='".(int)$data['panel_id']."'");
	$result = dbquery("UPDATE ".DB_PANELS." SET panel_order=panel_order-1 WHERE panel_id='".(int)$_GET['panel_id']."'");
	redirect(FUSION_SELF.$aidlink);
}
...
if ((isset($_GET['action']) && $_GET['action'] == "mleft") && (isset($_GET['panel_id']) && isnum($_GET['panel_id']))) {
	...
	$result = dbquery("UPDATE ".DB_PANELS." SET panel_side='1', panel_order='".(int)$neworder."' WHERE panel_id='".(int)$_GET['panel_id']."'");
...
if ((isset($_GET['action']) && $_GET['action'] == "mright") && (isset($_GET['panel_id']) && isnum($_GET['panel_id']))) {
	...
	if (dbrows($result) != 0) { $data = dbarray($result); $neworder = $data['panel_order'] + 1; } else { $neworder = 1; }
	$result = dbquery("UPDATE ".DB_PANELS." SET panel_side='4', panel_order='".(int)$neworder."' WHERE panel_id='".(int)$_GET['panel_id']."'");
...
if ((isset($_GET['action']) && $_GET['action'] == "mupper") && (isset($_GET['panel_id']) && isnum($_GET['panel_id']))) {
	...
	if (dbrows($result) != 0) { $data = dbarray($result); $neworder = $data['panel_order'] + 1; } else { $neworder = 1; }
	$result = dbquery("UPDATE ".DB_PANELS." SET panel_side='2', panel_order='".(int)$neworder."' WHERE panel_id='".(int)$_GET['panel_id']."'");
...
if ((isset($_GET['action']) && $_GET['action'] == "mlower") && (isset($_GET['panel_id']) && isnum($_GET['panel_id']))) {
	...
	if (dbrows($result) != 0) { $data = dbarray($result); $neworder = $data['panel_order'] + 1; } else { $neworder = 1; }
	$result = dbquery("UPDATE ".DB_PANELS." SET panel_side='3', panel_order='".(int)$neworder."' WHERE panel_id='".(int)$_GET['panel_id']."'");
...
echo "</tr>\n";
$ps = 1; $i = 1; $k = 0;
$result = dbquery("SELECT panel_id, panel_name, panel_filename, panel_side, panel_order, panel_type, panel_access, panel_status FROM ".DB_PANELS." ORDER BY panel_side, panel_order");
while ($data = dbarray($result)) {
	if($data['panel_type'] == "file") $used_panels[] = $data['panel_filename']; // Store Used Panel
	...
	$image = ($data['panel_status'] == 1) ? get_image("active", $locale['426'], "", $locale['426']) : get_image("inactive", $locale['427'], "", $locale['427']); // Pimped
	echo "<td align='center' width='1%' class='".$row_color."' style='white-space:nowrap'>".$image."</td>"; // Pimped
...
$i = 0;
$show_panel_out = "";
foreach ($panel_list as $value) {
    if(!in_array($value, $used_panels)) {
		$row_color = ($i % 2 == 0 ? "tbl1" : "tbl2");
		$show_panel_out .= "<tr>\n";
		$show_panel_out .= "<td class='".$row_color."'>".$value."</td>\n";
		$show_panel_out .= "<td class='".$row_color."'>[<a href='panel_editor.php".$aidlink."&amp;filename=".$value."'>".$locale['493']."</a>]</td>\n";
		$show_panel_out .= "</tr>\n";
		$i++;
...
opentable($locale['490']);
	...
	echo "<tr>\n";
	echo "<td class='tbl2'><strong>".$locale['491']."</strong></td>\n";
	echo "<td class='tbl2'><strong>".$locale['492']."</strong></td>\n";
	echo "</tr>\n";
	echo $show_panel_out;
	echo "</table>\n";
closetable();
