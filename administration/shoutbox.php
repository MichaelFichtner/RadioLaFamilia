...
if (isset($_GET['status']) && !isset($message)) {
	if ($_GET['status'] == "su") {
		...
		if ($_GET['error'] == 1) { $message .= "Shout contains no text</span>"; }
	}
...
if (isset($_GET['action']) && $_GET['action'] == "deleteshouts") {
	...
	$count = dbcount("(shout_id)", DB_SHOUTBOX); 
	...
	if ($count != 0) {
		...
		$i = 0;
		$result = dbquery(
			"SELECT s.shout_id, s.shout_name, s.shout_message, s.shout_datestamp, s.shout_ip, u.user_name, u.user_status
			FROM ".DB_SHOUTBOX." s
		...
		while ($data = dbarray($result)) {
			...
			if ($data['user_name']) {
				echo "<span class='slink'>".profile_link($data['shout_name'], $data['user_name'], $data['user_status'])."</span>";
			} else {
	...
	closetable();
	echo "<div align='center' style='margin-top:5px;'>\n".pagination(true,$_GET['rowstart'],20,$count,3,FUSION_SELF.$aidlink."&amp;")."\n</div>\n"; // Pimped
}
