...
if (isset($_GET['status']) && !isset($message)) {
	if ($_GET['status'] == "sn") {
		...
		if ($_GET['error'] == 1) { $message .= "No Cat Name entered</span>"; }
	}
...
if (isset($_GET['action']) && $_GET['action'] == "refresh") {
	...
	echo "</tr>\n";
	$result = dbquery("SELECT field_cat_id, field_cat_name, field_cat_order FROM ".DB_USER_FIELD_CATS." ORDER BY field_cat_order");
	if (dbrows($result)) {
		$i = 0; $k = 1;
		while($data = dbarray($result)) {
