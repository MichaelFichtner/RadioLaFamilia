...
include LOCALE.LOCALESET."admin/mods.php";

if (!checkrights("GM") || !defined("iAUTH") || $_GET['aid'] != iAUTH) { redirect("../index.php"); }
...
if (isset($_POST['add_admin']) && (isset($_POST['user_id']) && isnum($_POST['user_id']))) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		if (isset($_POST['all_rights'])) {
			$admin_rights = "C.S";
			$result = dbquery("UPDATE ".DB_USERS." SET user_level='".nMODERATOR."', user_rights='$admin_rights' WHERE user_id='".$_POST['user_id']."'");
		}
		log_admin_action("admin-2", "admin_moderator_added", "", "", $_POST['user_id']); // Log Admin's Action
...
if (isset($_GET['remove']) && (isset($_GET['remove']) && isnum($_GET['remove']) && $_GET['remove'] != 1)) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		$result = dbquery("UPDATE ".DB_USERS." SET user_admin_password='', user_level='".nMEMBER."', user_rights='' WHERE user_id='".$_GET['remove']."' AND user_level='".nMODERATOR."'");
		log_admin_action("admin-2", "admin_moderator_removed", "", "", $_GET['remove']); // Log Admin's Action
		set_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "");
	...
	$i = 0;
	$result = dbquery("SELECT * FROM ".DB_USERS." WHERE user_level='".nMODERATOR."' ORDER BY user_level DESC, user_name");
	echo "<table cellpadding='0' cellspacing='1' width='450' class='tbl-border center'>\n<tr>\n";
