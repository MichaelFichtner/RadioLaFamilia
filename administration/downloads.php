...
$count = dbcount("(download_cat_id)", DB_DOWNLOAD_CATS);
if ($count != 0) {
	if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['download_id']) && isnum($_GET['download_id']))) {
		$result = dbquery("SELECT download_title, download_file FROM ".DB_DOWNLOADS." WHERE download_id='".$_GET['download_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
			...
			log_admin_action("admin-1", "admin_dl_deleted", "", "", $data['download_title']." (ID: ".$_GET['download_id'].")");
		}
		...
		if (isset($_POST['del_upload']) && isset($_GET['download_id']) && isnum($_GET['download_id'])) {
			...
			$download_file = "";
			$download_url = (isset($_POST['download_url']) && $_POST['download_url'] != "") ? stripinput($_POST['download_url']) : "";
		...
		} elseif (isset($_POST['download_file']) && $_POST['download_file'] != "" && isset($_GET['download_id']) && isnum($_GET['download_id'])) {
		...
		if ($download_title && $error == 0) {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['download_id']) && isnum($_GET['download_id']))) {
				...
				$result = dbquery("UPDATE ".DB_DOWNLOADS." SET download_title='$download_title', download_description='$download_description', download_url='$download_url', download_file="._db($download_file).", download_cat='$download_cat', download_license='$download_license', download_os='$download_os', download_version='$download_version', download_filesize='$download_filesize'".$download_datestamp." WHERE download_id='".$_GET['download_id']."'");
				log_admin_action("admin-1", "admin_dl_edited", "", "", $download_title." (ID: ".$_GET['download_id'].")");
				redirect(FUSION_SELF.$aidlink."&download_cat_id=".$download_cat."&status=su");
			} else {
				$result = dbquery("INSERT INTO ".DB_DOWNLOADS." (download_title, download_description, download_url, download_file, download_cat, download_license, download_os, download_version, download_filesize, download_datestamp, download_count) VALUES ('$download_title', '$download_description', '$download_url', "._db($download_file).", '$download_cat', '$download_license', '$download_os', '$download_version', '$download_filesize', '".time()."', '0')");
				$dl_id = mysql_insert_id();
				log_admin_action("admin-1", "admin_dl_added", "", "", $download_title." (ID: ".$dl_id.")");
	...
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['download_id']) && isnum($_GET['download_id']))) {
		$result = dbquery("SELECT download_title, download_description, download_url, download_file, download_cat,
		download_license, download_os, download_version, download_filesize
	...
	$editlist = ""; $sel = "";
	if (isset($_GET['action']) && $_GET['action'] == "edit") { $edited_id = $data['download_cat']; } else { $edited_id = ""; }
	$result2 = dbquery("SELECT download_cat_id, download_cat_name
	FROM ".DB_DOWNLOAD_CATS." WHERE download_cat_parent='0' ORDER BY download_cat_name");
	if (dbrows($result2) != 0) {
		while ($data2 = dbarray($result2)) {
		 ...
		 $editlist .= editlist_sub($data2['download_cat_id'], $edited_id); // Pimped
		 }
	  }
	...
	if (!empty($download_file)) {
		...
		echo "<label><input type='checkbox' name='calc_upload' id='calc_upload' checked='checked' value='1' /> ".$locale['432']."</label>\n";
	}
	echo "</td>\n</tr>\n<tr>\n";
	...
	if (dbrows($result)) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<td class='tbl2' align='right'><img src='".get_image("panel_$p_img")."' name='b_".$flipbox_id.$data['download_cat_id']."' alt='' onclick=\"javascript:flipBox('".$flipbox_id.$data['download_cat_id']."')\" /></td>\n";
			echo "</tr>\n";
			...
			if (dbrows($result2) != 0) {
				echo "<tr>\n<td colspan='2'>\n";
				echo "<div id='box_".$flipbox_id.$data['download_cat_id']."'".$div.">\n";
				...
				echo "<tr>\n<td colspan='2'>\n";
				echo "<div id='box_".$flipbox_id.$data['download_cat_id']."' style='display:none'>\n";
				echo "<table width='100%' cellspacing='0' cellpadding='0'>\n<tr>\n";
				echo "<td class='tbl'>"."There are no downloads saved in this category"."</td>\n";
				echo "</tr>\n</table>\n</div>\n</td>\n</tr>\n";
			}
			echo subcats($data['download_cat_id']); // Pimped: changed
		}
		echo "</table>\n";
	} else {
		echo "<tr>\n<td align='center'><br />\n";
...
//Pimped: subcategory begin
function subcats($id) {
	global $aidlink, $locale, $flipbox_id;
	$result = dbquery("SELECT download_cat_id, download_cat_name FROM ".DB_DOWNLOAD_CATS." WHERE download_cat_parent='".(int)$id."' ORDER BY download_cat_name");//pimped
	$sublist = "";
	while ($data_p = dbarray($result)) {
		if (!isset($_GET['download_cat_id']) || !isnum($_GET['download_cat_id'])) { $_GET['download_cat_id'] = 0; }
		if ($data_p['download_cat_id'] == $_GET['download_cat_id']) { $p_img = "off"; $div = ""; } else { $p_img = "on"; $div = "style='display:none'"; }
		$sublist .= "<tr>\n";
		$sublist .= "<td class='tbl2'>--".$data_p['download_cat_name']."</td>\n";
		$sublist .= "<td class='tbl2' align='right'><img src='".get_image("panel_$p_img")."' name='b_".$flipbox_id.$data_p['download_cat_id']."' alt='' onclick=\"javascript:flipBox('".$flipbox_id.$data_p['download_cat_id']."')\" /></td>\n";
		$sublist .= "</tr>\n";
		$result2 = dbquery("SELECT download_id, download_title, download_url, download_file FROM ".DB_DOWNLOADS." WHERE download_cat='".$data_p['download_cat_id']."' ORDER BY download_title");
		if (dbrows($result2)) {
			$sublist .= "<tr>\n<td colspan='2'>\n";
			$sublist .= "<div id='box_".$flipbox_id.$data_p['download_cat_id']."'".$div.">\n";
			$sublist .= "<table cellpadding='0' cellspacing='0' width='100%'>\n";
			while ($data_parent = dbarray($result2)) {
				if (!empty($data_parent['download_file']) && file_exists(DOWNLOADS.$data_parent['download_file'])) {
					$download_url = DOWNLOADS.$data_parent['download_file'];
				} elseif (!strstr($data_parent['download_url'],"http://") && !strstr($data_parent['download_url'],"../")) {
					$download_url = BASEDIR.$data_parent['download_url'];
				} else {
					$download_url = $data_parent['download_url'];
				}
				$sublist .= "<tr>\n<td class='tbl'><a href='$download_url' target='_blank'>".$data_parent['download_title']."</a></td>\n";
				$sublist .= "<td align='right' width='100' class='tbl'><a href='".FUSION_SELF.$aidlink."&amp;action=edit&amp;download_cat_id=".$data_p['download_cat_id']."&amp;download_id=".$data_parent['download_id']."'>".$locale['442']."</a> -\n";
				$sublist .= "<a href='".FUSION_SELF.$aidlink."&amp;action=delete&amp;download_cat_id=".$data_p['download_cat_id']."&amp;download_id=".$data_parent['download_id']."' onclick=\"return confirm('".$locale['460']."');\">".$locale['443']."</a></td>\n";
				$sublist .= "</tr>\n";
              }
			  $sublist .= "</table>\n</div>\n</td></tr>\n";
		 } else {
			$sublist .= "<tr>\n<td colspan='2'>\n";
			$sublist .= "<div id='box_".$flipbox_id.$data_p['download_cat_id']."' style='display:none'>\n";
			$sublist .= "<table width='100%' cellspacing='0' cellpadding='0'>\n<tr>\n";
			$sublist .= "<td class='tbl'>"."There are no downloads saved in this subcategory"."</td>\n";
			$sublist .= "</tr>\n</table>\n</div>\n</td>\n</tr>\n";
		}
    }
	return $sublist;
...
function editlist_sub($id, $edited_id) {
	global $locale;
 $result = dbquery("SELECT download_cat_id, download_cat_parent, download_cat_name FROM ".DB_DOWNLOAD_CATS." WHERE download_cat_parent='".(int)$id."' ORDER BY download_cat_name");//pimped
	$catlist = ""; $sel = "";
	$k = dbrows($result);
	if ($k > 0) {
		while ($data = dbarray($result)) {
			if (isset($_GET['action']) && $_GET['action'] == "edit") { $sel = ($edited_id == $data['download_cat_id'] ? " selected='selected'" : ""); }
			$catlist .= "<option value='".$data['download_cat_id']."'".$sel.">--".$data['download_cat_name']."</option>\n"; // Pimped
		}
	}
    return $catlist;
}
