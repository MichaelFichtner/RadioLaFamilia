...
if (function_exists('gd_info')) {
	...
	if (isset($_GET['action']) && $_GET['action'] == "refresh") {
		$i = 1; $k = 1;
		$result = dbquery("SELECT album_id FROM ".DB_PHOTO_ALBUMS." WHERE album_parent='0' ORDER BY album_order"); // Pimped: Subcategories
	...
	if (isset($_GET['status']) && !isset($message)) {
		if ($_GET['status'] == "sn") {
			...
			elseif ($_GET['error'] == 5) { $message .= "No Album title entered</span>"; }
	...
	if (isset($_POST['cancel'])) {
			...
			if (!isset($_POST['confirm_password'])) {
				...
				if (check_admin_pass(isset($_POST['admin_passwd']) ? stripinput($_POST['admin_passwd']) : "")) {
					if (dbcount("(album_id)", DB_PHOTOS, "album_id='".$_GET['album_id']."'")) {
						$result = dbquery("SELECT album_id, photo_thumb1, photo_thumb2, photo_filename FROM ".DB_PHOTOS." WHERE album_id='".(int)$_GET['album_id']."'");
						while ($data = dbarray($result)) {
							if (!SAFEMODE) {
				...
				$data = dbarray(dbquery("SELECT album_id, album_thumb, album_order FROM ".DB_PHOTO_ALBUMS." WHERE album_id='".$_GET['album_id']."'"));
		...
		$data = dbarray(dbquery("SELECT album_id FROM ".DB_PHOTO_ALBUMS." WHERE album_order='".intval($_GET['order'])."' AND album_parent='0'"));
		...
		redirect(FUSION_SELF.$aidlink."&page=$rowstart"); // Pimped
		...
		$album_parent = isnum($_POST['album_parent']) ? $_POST['album_parent'] : "0"; // pimped subcategories
		...
		if($album_title != "") {
			...
			if (!$error) {
				if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['album_id']) && isnum($_GET['album_id']))) {
					...
					".(isset($album_thumb) ? " album_thumb='$album_thumb'," : "")." album_user='".$userdata['user_id']."',
					...
					redirect(FUSION_SELF.$aidlink."&status=su&page=$rowstart"); // Pimped
				} else {
					...
					$rowstart = $album_order > $settings['thumbs_per_page'] ? 
					((ceil($album_order / $settings['thumbs_per_page']) - 1 ) * $settings['thumbs_per_page']) : "0";
					redirect(FUSION_SELF.$aidlink."&status=sn&page=$rowstart"); // Pimped
				}
			} else {
		...
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['album_id']) && isnum($_GET['album_id']))) {
			$result = dbquery("SELECT album_title, album_description, album_thumb, album_parent, album_access, album_order
			FROM ".DB_PHOTO_ALBUMS." WHERE album_id='".$_GET['album_id']."'"); // Pimped: subcategories
			if (dbrows($result)) {
				$data = dbarray($result);
				$album_title = $data['album_title'];
				$album_description = $data['album_description'];
				$album_thumb = $data['album_thumb'];
				$album_parent = $data['album_parent']; // Pimped: subcategories
			...
			$album_description = "";
			$album_thumb = "";
			$album_parent = 0;//pimped subcategories
			$album_access = "";
			$album_order = "";
	...
	$rows = dbcount("(album_id)", DB_PHOTO_ALBUMS, "album_parent='0'");
	if ($rows) {
		...
		$result = dbquery(
			"SELECT ta.album_id, ta.album_title, ta.album_thumb, ta.album_order, ta.album_datestamp, ta.album_access,
			tu.user_id, tu.user_name, tu.user_status
			FROM ".DB_PHOTO_ALBUMS." ta
			LEFT JOIN ".DB_USERS." tu ON ta.album_user=tu.user_id
			WHERE album_parent='0'
		...
		while ($data = dbarray($result)) {
			...
			if ($rows != 1){
				...
				if($k == 1){
					$down = " &middot;\n<a href='".FUSION_SELF.$aidlink."&amp;page=".$_GET['rowstart']."&amp;action=mdown&amp;order=$orderd&amp;album_id=".$data['album_id']."'><img src='".get_image("right")."' alt='".$locale['467']."' title='".$locale['468']."' style='border:0px;vertical-align:middle' /></a>\n";
				} elseif($k < $rows){
					$up = "<a href='".FUSION_SELF.$aidlink."&amp;page=".$_GET['rowstart']."&amp;action=mup&amp;order=$orderu&amp;album_id=".$data['album_id']."'><img src='".get_image("left")."' alt='".$locale['467']."' title='".$locale['466']."' style='border:0px;vertical-align:middle' /></a> &middot;\n";
					$down = " &middot;\n<a href='".FUSION_SELF.$aidlink."&amp;page=".$_GET['rowstart']."&amp;action=mdown&amp;order=$orderd&amp;album_id=".$data['album_id']."'><img src='".get_image("right")."' alt='".$locale['467']."' title='".$locale['468']."' style='border:0px;vertical-align:middle' /></a>\n";
				} else {
					$up = "<a href='".FUSION_SELF.$aidlink."&amp;page=".$_GET['rowstart']."&amp;action=mup&amp;order=$orderu&amp;album_id=".$data['album_id']."'><img src='".get_image("left")."' alt='".$locale['467']."' title='".$locale['466']."' style='border:0px;vertical-align:middle' /></a> &middot;\n";
				}
			...
			if ($data['album_thumb'] && file_exists(PHOTOS.$data['album_thumb'])){
				...
				echo "<img src='".PHOTOS."nophoto.jpg' alt='' title='".$locale['460']."' style='border:0px' />";
			} else {
				echo $locale['461'];
			}
			$sub = dbcount("(album_id)", DB_PHOTO_ALBUMS, "album_parent='".$data['album_id']."'"); // Pimped: Subcategories
			echo "</a><br /><br />\n<span class='small'>".$up;
			...
			echo "<br /><br />\n";
			echo $locale['447'].$sub."<br />"; // Pimped: Subcategories
			echo $locale['462'].showdate("shortdate", $data['album_datestamp'])."<br />\n";
