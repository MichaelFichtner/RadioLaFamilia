...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['faq_cat_id']) && isnum($_GET['faq_cat_id'])) && (isset($_GET['t']) && $_GET['t'] == "cat")) {
	$faq_count = dbcount("(faq_id)", DB_FAQS, "faq_cat_id='".(int)$_GET['faq_cat_id']."'");
	if ($faq_count > 0) {
		redirect(FUSION_SELF.$aidlink."&status=delcn");
	} else {
		$result = dbquery("SELECT faq_cat_name FROM ".DB_FAQ_CATS." WHERE faq_cat_id='".(int)$_GET['faq_cat_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
		}
		$result = dbquery("DELETE FROM ".DB_FAQ_CATS." WHERE faq_cat_id='".(int)$_GET['faq_cat_id']."'");
		log_admin_action("admin-1", "admin_faqcat_deleted", "", "", $data['faq_cat_name']." (ID: ".$_GET['faq_cat_id'].")");
	...
	$result = dbquery("SELECT faq_question FROM ".DB_FAQS." WHERE faq_id='".(int)$_GET['faq_id']."'");
	if (dbrows($result)) {
		$data = dbarray($result);
	}
	$result = dbquery("DELETE FROM ".DB_FAQS." WHERE faq_id='".(int)$_GET['faq_id']."'");
	log_admin_action("admin-1", "admin_faq_deleted", "", "", $data['faq_question']." (ID: ".$_GET['faq_id'].")");
	if (isset($_GET['faq_cat_id'])) {
	...
	if(IF_MULTI_LANGUAGE) { $faq_cat_language = stripinput($_POST['faq_cat_language']); } else { $faq_cat_language = true; }
	if ($faq_cat_name && $faq_cat_language) {
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['faq_cat_id']) && isnum($_GET['faq_cat_id'])) && (isset($_GET['t']) && $_GET['t'] == "cat")) {
			if(IF_MULTI_LANGUAGE) { $insert_mysql = ", faq_cat_language='$faq_cat_language'"; } else { $insert_mysql = ''; }
			$result = dbquery("UPDATE ".DB_FAQ_CATS." SET faq_cat_name='$faq_cat_name', faq_cat_description='$faq_cat_description'".$insert_mysql." WHERE faq_cat_id='".(int)$_GET['faq_cat_id']."'");
			log_admin_action("admin-1", "admin_faqcat_edited", "", "", $faq_cat_name." (ID: ".$_GET['faq_cat_id'].")");
			redirect(FUSION_SELF.$aidlink."&status=scu");
		} else {
			if(IF_MULTI_LANGUAGE){ $insert_mysql1 = ", faq_cat_language"; $insert_mysql2 = ", '$faq_cat_language'"; } else { $insert_mysql1 = ''; $insert_mysql2 = ''; }
			$result = dbquery("INSERT INTO ".DB_FAQ_CATS." (faq_cat_name, faq_cat_description".$insert_mysql1.") VALUES('$faq_cat_name', '$faq_cat_description'".$insert_mysql2.")");
			$cat_id = mysql_insert_id();
			log_admin_action("admin-1", "admin_faqcat_added", "", "", $faq_cat_name." (ID: ".$cat_id.")");
			redirect(FUSION_SELF.$aidlink."&status=scn");
		}
	} else {
		redirect(FUSION_SELF.$aidlink); // todo: Error-Msg
	}
	...
	if ($faq_question && $faq_answer) {
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['faq_id']) && isnum($_GET['faq_id'])) && (isset($_GET['t']) && $_GET['t'] == "faq")) {
			$result = dbquery("UPDATE ".DB_FAQS." SET faq_cat_id='$faq_cat', faq_question='$faq_question', faq_answer='$faq_answer' WHERE faq_id='".(int)$_GET['faq_id']."'");
			log_admin_action("admin-1", "admin_faq_edited", "", "", $faq_question." (ID: ".$_GET['faq_id'].")");
			...
			log_admin_action("admin-1", "admin_faq_added", "", "", $faq_question." (ID: ".$faq_id.")");
	...
	if ((isset($_GET['faq_cat_id']) && isnum($_GET['faq_cat_id'])) && (isset($_GET['t']) && $_GET['t'] == "cat")) {
		$result = dbquery("SELECT faq_cat_id, faq_cat_name, faq_cat_description, faq_cat_language FROM ".DB_FAQ_CATS." WHERE faq_cat_id='".(int)$_GET['faq_cat_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
			$faq_cat_id = $data['faq_cat_id'];
			$faq_cat_name = $data['faq_cat_name'];
			$faq_cat_description = $data['faq_cat_description'];
			if(IF_MULTI_LANGUAGE) $faq_cat_language = $data['faq_cat_language'];
		...
		$result = dbquery("SELECT faq_id, faq_question, faq_answer FROM ".DB_FAQS." WHERE faq_id='".(int)$_GET['faq_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
			$faq_cat_name = "";
			$faq_cat_description = "";
...
if (!isset($_GET['t']) || $_GET['t'] != "faq") {
	...
	echo "<td width='130' class='tbl'>".$locale['524']."</td>\n";
	$opts = make_admin_language_opts($faq_cat_language);
	echo "<td class='tbl'><select name='faq_cat_language' class='textbox' style='width:200px;'>\n".$opts."</select></td>\n";
	echo "</tr>\n<tr>\n";
	}
...
if (!isset($_GET['t']) || $_GET['t'] != "cat") {
	$cat_opts = ""; $sel = "";
	$result2 = dbquery("SELECT faq_cat_id, faq_cat_name, faq_cat_language FROM ".DB_FAQ_CATS." ORDER BY faq_cat_name");
	if (dbrows($result2) != 0) {
		while ($data2 = dbarray($result2)) {
			...
			$cat_opts .= "<option value='".$data2['faq_cat_id']."'$sel>".$data2['faq_cat_name']." (".$locale['524'].": ".$data2['faq_cat_language'].")</option>\n";
		}
		opentable($faq_title);
...
$result = dbquery("SELECT faq_cat_id, faq_cat_name, faq_cat_language FROM ".DB_FAQ_CATS." ORDER BY faq_cat_name");
if (dbrows($result) != 0) {
	...
	while ($data = dbarray($result)) {
		...
		echo "<tr>\n";
		echo "<td class='tbl2'><img src='".get_image("panel_$p_img")."' name='b_".$data['faq_cat_id']."' alt='' onclick=\"javascript:flipBox('".$data['faq_cat_id']."')\" /> ".$data['faq_cat_name'];
		if(IF_MULTI_LANGUAGE) echo " (".$locale['524'].": ".$data['faq_cat_language'].")";
		echo "</td>\n";
		...
		echo "</tr>\n";
		$result2 = dbquery("SELECT faq_id, faq_question, faq_answer FROM ".DB_FAQS." WHERE faq_cat_id='".(int)$data['faq_cat_id']."' ORDER BY faq_id");
		if (dbrows($result2) != 0) {
			echo "<tr>\n<td colspan='2'>\n";
