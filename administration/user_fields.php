...
if (isset($_GET['status']) && !isset($message)) {
	if ($_GET['status'] == "fld_on") {
		$message = $locale['460'];
	} elseif ($_GET['status'] == "fld_off") {
		$message = $locale['461'];
	} elseif ($_GET['status'] == "fld_edit") {
		$message = $locale['462'];
	} elseif ($_GET['status'] == "fld_failed") {
		$message = $locale['463'];
	}
	if (isset($message)) {
...
if ((isset($_GET['action']) && $_GET['action'] == "mu") && (isset($_GET['field_id']) && isnum($_GET['field_id']))) {
	...
	} elseif(file_exists(LOCALE."English/user_fields/".stripinput($_GET['enable']).".php")) { // Pimped: include English 
		include LOCALE."English/user_fields/".stripinput($_GET['enable']).".php";
	}
	...
	if (isset($_POST['enable'])) {
		...
		if (dbrows($result)) {
			...
			$result1 = dbquery("UPDATE ".DB_USER_FIELDS." SET field_cat='".$field_cat."', field_order='".$field_order."' WHERE field_name='".stripinput($_GET['enable'])."'");
			$result2 = dbquery("UPDATE ".DB_USER_FIELDS." SET field_order=field_order-1 WHERE field_cat='".$data['field_cat']."' AND field_order>'".$data['field_order']."'");
			redirect(FUSION_SELF.$aidlink.($result1 ? "&status=fld_edit" : "&status=fld_failed"));
		} else {
			$field_order = dbresult(dbquery("SELECT MAX(field_order) FROM ".DB_USER_FIELDS." WHERE field_cat='".$field_cat."'"), 0) + 1;
			if (!$user_field_dbinfo || $result = dbquery("ALTER TABLE ".DB_USERS." ADD ".$user_field_dbname." ".$user_field_dbinfo)) {
				$result1 = dbquery("INSERT INTO ".DB_USER_FIELDS." (field_name, field_cat, field_order) VALUES ('".$user_field_dbname."', '".$field_cat."', '".$field_order."')");
				redirect(FUSION_SELF.$aidlink.($result1 ? "&status=fld_on" : "&status=fld_failed"));
			} else {
				redirect(FUSION_SELF.$aidlink."&status=fld_failed");
			}
		}
	} else {
	...
	redirect(FUSION_SELF.$aidlink.($error ? "&status=fld_failed" : "&status=fld_off"));
}
...
$result = dbquery(
	"SELECT tuf.field_id, tuf.field_name, tuf.field_cat, tuf.field_order,
	tufc.field_cat_name
...
if (dbrows($result)) {
	...
	while($data = dbarray($result)) {
		...
		if (file_exists(LOCALE.LOCALESET."user_fields/".$data['field_name'].".php")) {
			include LOCALE.LOCALESET."user_fields/".$data['field_name'].".php";
		} elseif(file_exists(LOCALE."English/user_fields/".$data['field_name'].".php")) { // Pimped: include English 
			include LOCALE."English/user_fields/".$data['field_name'].".php";
		} else {
			#echo "The Locale file of ".$data['field_name']." could not be included!"; // Debuging for Developer
		}
		include INCLUDES."user_fields/".$data['field_name']."_include_var.php";
...
if (count($available_fields) != count($enabled_fields)) {
	...
	for ($k = 0; $k < count($available_fields); $k++) {
		if (!in_array($available_fields[$k], $enabled_fields)) {
			...
			} elseif(file_exists(LOCALE."English/user_fields/".$available_fields[$k].".php")) { // Pimped: include English 
				include LOCALE."English/user_fields/".$available_fields[$k].".php";
			}
