...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
	$count = dbcount("(download_cat)", DB_DOWNLOADS, "download_cat='".(int)$_GET['cat_id']."'");
	if ($count != 0) {
		redirect(FUSION_SELF.$aidlink."&status=deln");
	} else {
		$result = dbquery("SELECT download_cat_name FROM ".DB_DOWNLOAD_CATS." WHERE download_cat_id='".(int)$_GET['cat_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
		}
		$result = dbquery("DELETE FROM ".DB_DOWNLOAD_CATS." WHERE download_cat_id='".$_GET['cat_id']."'");
		log_admin_action("admin-1", "admin_dlcat_deleted", "", "", $data['download_cat_name']." (ID: ".$_GET['cat_id'].")");
	...
	if (isset($_POST['save_cat'])) {
		...
		if ($cat_name) {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
				$result = dbquery("UPDATE ".DB_DOWNLOAD_CATS." SET download_cat_name='$cat_name', download_cat_description='$cat_description', download_cat_sorting='$cat_sorting', download_cat_parent='$cat_parent', download_cat_access='$cat_access' WHERE download_cat_id='".$_GET['cat_id']."'");
				log_admin_action("admin-1", "admin_dlcat_edited", "", "", $cat_name." (ID: ".$_GET['cat_id'].")");
				redirect(FUSION_SELF.$aidlink."&status=su");
			} else {
				$result = dbquery("INSERT INTO ".DB_DOWNLOAD_CATS." (download_cat_name, download_cat_description, download_cat_sorting, download_cat_parent, download_cat_access) VALUES('$cat_name', '$cat_description', '$cat_sorting', '$cat_parent', '$cat_access')");
				$cat_id = mysql_insert_id();
				log_admin_action("admin-1", "admin_dlcat_added", "", "", $cat_name." (ID: ".$cat_id.")");
	...
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
		$result = dbquery("SELECT download_cat_id, download_cat_name, download_cat_description, download_cat_sorting, download_cat_parent, download_cat_access FROM ".DB_DOWNLOAD_CATS." WHERE download_cat_id='".(int)$_GET['cat_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
			...
			$cat_parent = $data['download_cat_parent']; // Pimped
	...
	$editlist = ""; $sel = "";
	$result2 = dbquery("SELECT download_cat_id, download_cat_name FROM ".DB_DOWNLOAD_CATS." WHERE download_cat_parent='0' ".((isset($_GET['cat_id']) && isnum($_GET['cat_id'])) ? "AND download_cat_id!='".(int)$_GET['cat_id']."' " : "")."ORDER BY download_cat_name");
	if (dbrows($result2) != 0) {
	...
	$result = dbquery("SELECT download_cat_id, download_cat_name, download_cat_description, download_cat_access
	...
	if (dbrows($result) != 0) {
		...
		while ($data = dbarray($result)) {
			...
			if ($data['download_cat_description']) { echo "\n<br /><span class='small'>".trimlink($data['download_cat_description'], 45)."</span>\n"; }
			echo "</td>\n";
			...
			echo "</tr>\n";
			echo subcats($data['download_cat_id']);//pimped
			$i++;
