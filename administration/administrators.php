...
if (isset($_POST['add_admin']) && (isset($_POST['user_id']) && isnum($_POST['user_id']))) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		if (isset($_POST['all_rights']) || isset($_POST['make_super'])) {
			...
			$result = dbquery("UPDATE ".DB_USERS." SET user_level='".(isset($_POST['make_super']) ? nSUPERADMIN : nADMIN)."', user_rights='$admin_rights' WHERE user_id='".$_POST['user_id']."'");
		} else {
			if(isset($_POST['forum_mod_rights'])) {
				$result = dbquery("UPDATE ".DB_USERS." SET user_level='".nADMIN."', user_rights='FMD' WHERE user_id='".$_POST['user_id']."'");
			} else {
				$result = dbquery("UPDATE ".DB_USERS." SET user_level='".nADMIN."' WHERE user_id='".$_POST['user_id']."'");
			}
		}
		log_admin_action("admin-2", "admin_adminrights_added", "", "", $_POST['user_id']); // Log Admin's Action
...
if (isset($_GET['remove']) && (isset($_GET['remove']) && isnum($_GET['remove']) && $_GET['remove'] != 1)) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		$result = dbquery("UPDATE ".DB_USERS." SET user_admin_password='', user_level='".nMEMBER."', user_rights='' WHERE user_id='".$_GET['remove']."' AND user_level>='".nADMIN."'");
		log_admin_action("admin-2", "admin_adminrights_removed", "", "", $_GET['remove']); // Log Admin's Action
		set_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "");
...
if (isset($_POST['update_admin']) && (isset($_GET['user_id']) && isnum($_GET['user_id']) && $_GET['user_id'] != 1)) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		if (isset($_POST['rights'])) {
			...
			$result = dbquery("UPDATE ".DB_USERS." SET user_rights='$user_rights' WHERE user_id='".$_GET['user_id']."' AND user_level>='".nADMIN."'");
		} else {
			$result = dbquery("UPDATE ".DB_USERS." SET user_rights='' WHERE user_id='".$_GET['user_id']."' AND user_level>='".nADMIN."'");
		}
		log_admin_action("admin-2", "admin_adminrights_changed", "", "", $_GET['user_id']); // Log Admin's Action
		set_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "");
...
if (isset($_GET['edit']) && isnum($_GET['edit']) && $_GET['edit'] != 1) {
	$result = dbquery("SELECT user_name, user_rights FROM ".DB_USERS." WHERE user_id='".$_GET['edit']."' AND user_level>='".nADMIN."' ORDER BY user_id");
	if (dbrows($result)) {
		$data = dbarray($result);
		...
		while ($data2 = dbarray($result2)) {
			...
			echo "<td width='50%' class='tbl1'><label><input type='checkbox' name='rights[]' value='".$data2['admin_rights']."'".
			(in_array($data2['admin_rights'], $user_rights) ? " checked='checked'" : "")." /> ";
	...
	if (!isset($_POST['search_users']) || !isset($_POST['search_criteria'])) {
		...
		if ($mysql_search) {
			$result = dbquery("SELECT user_id, user_name FROM ".DB_USERS." WHERE ".$mysql_search." AND user_level='".nMEMBER."' ORDER BY user_name");
		}
		if (isset($result) && dbrows($result)) {
			...
			if ($i > 0) {
				...
				echo $users."<tr>\n<td align='center' class='tbl'>\n";
				echo "<label><input type='checkbox' name='forum_mod_rights' value='1' /> "."add Forum Moderator Rights"."</label><br />\n";
				echo "<label><input type='checkbox' name='all_rights' value='1' /> ".$locale['415']."</label><br />\n";
				if ($userdata['user_level'] == nSUPERADMIN) { echo "<label><input type='checkbox' name='make_super' value='1' /> ".$locale['416']."</label><br />\n"; }
				if (!check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
					echo $locale['447']." <input type='password' name='admin_password' class='textbox' style='width:150px;' /><br /><br />\n";
				}
	...
	$i = 0;
	$result = dbquery("SELECT user_id, user_name, user_rights, user_level FROM ".DB_USERS." WHERE user_level>='".nADMIN."' ORDER BY user_level DESC, user_name");
	echo "<table cellpadding='0' cellspacing='1' width='450' class='tbl-border center'>\n<tr>\n";
	...
	while ($data = dbarray($result)) {
		...
		if ($data['user_level'] == nSUPERADMIN && $userdata['user_id'] == "1") { $can_edit = true;
		} elseif ($data['user_level'] != nSUPERADMIN) { $can_edit = true;
		} else { $can_edit = false; }
		if ($can_edit == true && $data['user_id'] != "1") {
			if($data['user_level'] != nSUPERADMIN) echo "<a href='".FUSION_SELF.$aidlink."&amp;edit=".$data['user_id']."'>".$locale['426']."</a> |\n"; //pimped
