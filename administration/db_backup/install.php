<?php
##########################################
include_once ('./access.php'); //// Pimped
##########################################
/**
...
if ($phase == 2) {
    ...
    // default values
	############################### Pimped -->
	if (!@require BASEDIR."config.php") die("<div style='font-family:Verdana;font-size:11px;text-align:center;'><strong>config.php not found!</strong><br /></div>");
    if (!isset($config['dbhost'])) $config['dbhost']=$db_host; #### Pimped
	if (!isset($config['dbuser'])) $config['dbuser']=$db_user; #### Pimped
	if (!isset($config['dbpass'])) $config['dbpass']=$db_pass; #### Pimped
	############################### <-- Pimped
