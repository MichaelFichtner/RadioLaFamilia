...
if ($_GET['page'] == 1) {
	if ((isset($_GET['action']) && $_GET['action'] == "mup") && (isset($_GET['bbcode_id']) && isnum($_GET['bbcode_id']))) {
		...
		$result = dbquery("SELECT bbcode_name FROM ".DB_BBCODES." WHERE bbcode_name="._db($_GET['enable']).""); // Pimped
		if(dbrows($result) == 0) { // Pimped
			if (substr($_GET['enable'], 0, 1)!='!') {
				...
				$count = dbcount("(bbcode_id)", DB_BBCODES);
				if ($count > 0) {
			...
			log_admin_action("admin-3", "admin_bbcode_enable", "", "", $_GET['enable']); // Log Admin's Action
			redirect (FUSION_SELF.$aidlink);
		} else {
		// give an error msg
		}
	} elseif (isset($_GET['disable']) && isnum($_GET['disable'])) {
		$result = dbquery("SELECT bbcode_name FROM ".DB_BBCODES." WHERE bbcode_id='".$_GET['disable']."'"); # added for Log
		$data_log = dbarray($result); # added for Log
		...
		log_admin_action("admin-3", "admin_bbcode_disable", "", "", $data_log['bbcode_name']); // Log Admin's Action
		redirect (FUSION_SELF.$aidlink);
	}
	...
	echo $navigation;
	$result = dbquery("SELECT bbcode_id, bbcode_name, bbcode_order FROM ".DB_BBCODES." ORDER BY bbcode_order");
	if (dbrows($result)) {
		...
		while ($data = dbarray($result)) {
			...
			if (file_exists(LOCALE.LOCALESET."bbcodes/".$data['bbcode_name'].".php")) {
				include_once LOCALE.LOCALESET."bbcodes/".$data['bbcode_name'].".php";
			} elseif (file_exists(LOCALE."English/bbcodes/".$data['bbcode_name'].".php")) {
				include_once LOCALE."English/bbcodes/".$data['bbcode_name'].".php";
      }
		...
		for ($lp=0; $lp < count($available_bbcodes); $lp++) {
			...
			if (!in_array($available_bbcodes[$lp], $enabled_bbcodes)) {
				...
				if (file_exists(LOCALE.LOCALESET."bbcodes/".$available_bbcodes[$lp].".php")) {
					include_once LOCALE.LOCALESET."bbcodes/".$available_bbcodes[$lp].".php";
				} elseif (file_exists(LOCALE."English/bbcodes/".$available_bbcodes[$lp].".php")) {
					include_once LOCALE."English/bbcodes/".$available_bbcodes[$lp].".php";
				}
	...
	if (isset($_POST['post_test'])) {
		...
		closetable();
		log_admin_action("admin-3", "admin_bbcode_tested"); // Log Admin's Action
	} else {
		$test_message = "";
		$smileys_checked = "";
	}
	include_once LOCALE.LOCALESET."comments.php";
