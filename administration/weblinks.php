...
if (isset($_GET['status']) && !isset($message)) {
	if ($_GET['status'] == "sn") {
		...
		$message = $locale['512'];
	} elseif ($_GET['status'] == "nwn") {
		$message = "No Weblink Name entered";
	} elseif ($_GET['status'] == "nwu") {
		$message = "No Weblink Url entered";
	} elseif ($_GET['status'] == "nwc") {
		$message = "No Weblink Cat entered";
	}
	if (isset($message)) { echo "<div id='close-message'><div class='admin-message'>".$message."</div></div>\n"; }
}

$count = dbcount("(weblink_cat_id)", DB_WEBLINK_CATS); 
if ($count != 0) {
	if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['weblink_id']) && isnum($_GET['weblink_id']))) {
		$log = dbresult(dbquery("SELECT weblink_name FROM ".DB_WEBLINKS." WHERE weblink_id='".(int)$_GET['weblink_id']."'"),0);
		$result = dbquery("DELETE FROM ".DB_WEBLINKS." WHERE weblink_id='".$_GET['weblink_id']."'");
		log_admin_action("admin-1", "admin_wl_deleted", "", "", $log." (ID: ".(int)$_GET['weblink_id'].")");
	...
	if (isset($_POST['save_link'])) {
		...
		if ($weblink_name && $weblink_cat && $weblink_url && $weblink_url != "http://") {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['weblink_id']) && isnum($_GET['weblink_id']))) {
				...
				log_admin_action("admin-1", "admin_wl_edited", "", "", $weblink_name." (ID: ".(int)$_GET['weblink_id'].")");
				...
				$id = mysql_insert_id();
				log_admin_action("admin-1", "admin_wl_added", "", "", $weblink_name." (ID: ".(int)$id.")");
				redirect(FUSION_SELF.$aidlink."&weblink_cat_id=$weblink_cat&amp;status=sn");
			}
		} else {
			if(!$weblink_name) {
				$status = "nwn";
			} elseif(!$weblink_url || $weblink_url == "http://") {
				$status = "nwu";
			} else {
				$status = "nwc";
			}
			redirect(FUSION_SELF.$aidlink."&weblink_cat_id=".$weblink_cat."&amp;status=".$status);
		}
	}
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['weblink_id']) && isnum($_GET['weblink_id']))) {
		$result = dbquery("SELECT weblink_name, weblink_description, weblink_url, weblink_cat
		FROM ".DB_WEBLINKS." WHERE weblink_id='".$_GET['weblink_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
	...
	$editlist = ""; $sel = "";
	$result2 = dbquery("SELECT weblink_cat_id, weblink_cat_name, weblink_cat_language
	...
	if (dbrows($result2) != 0) {
		while ($data2 = dbarray($result2)) {
			...
			$editlist .= "<option value='".$data2['weblink_cat_id']."'$sel>".$data2['weblink_cat_name']."  (".$locale['433']." ".$data2['weblink_cat_language'].")</option>\n"; //pimped
            $editlist .= editlist_sub($data2['weblink_cat_id']);//pimped
		}
	}
	...
	echo "<td class='tbl'><select name='weblink_cat' class='textbox' style='width:200px;'>\n".$editlist."</select></td>\n";
	echo "</tr>\n";
	echo "<tr>\n";
	...
	$result = dbquery("SELECT weblink_cat_id, weblink_cat_name, weblink_cat_language
	FROM ".DB_WEBLINK_CATS." WHERE weblink_cat_parent='0' ORDER BY weblink_cat_name");
	if (dbrows($result)) {
		echo "<tr>\n";
		...
		while ($data = dbarray($result)) {
			...
			echo "<tr>\n";
			echo "<td class='tbl2'>".$data['weblink_cat_name']." (".$locale['433']." ".$data['weblink_cat_language'].")</td>\n"; //pimped
			echo "<td class='tbl2' align='right'><img src='".get_image("panel_$p_img")."' alt='' name='b_".$flipbox_id.$data['weblink_cat_id']."' onclick=\"javascript:flipBox('".$flipbox_id.$data['weblink_cat_id']."')\" /></td>\n";
			echo "</tr>\n";
				
			$result2 = dbquery("SELECT weblink_id, weblink_name, weblink_url
			FROM ".DB_WEBLINKS." WHERE weblink_cat='".$data['weblink_cat_id']."' ORDER BY weblink_name");
			if (dbrows($result2)) {
				echo "<tr>\n<td colspan='2'>\n";
				echo "<div id='box_".$flipbox_id.$data['weblink_cat_id']."'".$div.">\n";
				...
				echo "<tr>\n<td colspan='2'>\n";
				echo "<div id='box_".$flipbox_id.$data['weblink_cat_id']."' style='display:none'>\n";
				echo "<table width='100%' cellspacing='0' cellpadding='0'>\n<tr>\n";
				echo "<td class='tbl'>".$locale['535']."</td>\n";
				echo "</tr>\n</table>\n</div>\n</td>\n</tr>\n";
			}
			
			echo subcats($data['weblink_cat_id']); // Pimped: changed
		}
		echo "</table>\n";
	}
	closetable();
...
function editlist_sub($id) {
    global $locale, $sel;
	$result = dbquery("SELECT weblink_cat_id, weblink_cat_name, weblink_cat_language FROM ".DB_WEBLINK_CATS." WHERE weblink_cat_parent='".$id."' ORDER BY weblink_cat_name");
	$catlist = "";
	$k = dbrows($result);
	if ($k > 0) {
		while ($data = dbarray($result)) {
			$catlist .= "<option value='".$data['weblink_cat_id']."'$sel>--".$data['weblink_cat_name'].(!(bool)IF_MULTI_LANGUAGE ? '':"  (".$locale['433']." ".$data['weblink_cat_language'].")")."</option>\n";
		}
	}
    return $catlist;	
}
...
function subcats($id) {
	...
	$result = dbquery("SELECT weblink_cat_id, weblink_cat_name, weblink_cat_language 
	FROM ".DB_WEBLINK_CATS." WHERE weblink_cat_parent='".(int)$id."' ORDER BY weblink_cat_name");
	$sublist = "";
	while ($data = dbarray($result)) {
		if (!isset($_GET['weblink_cat_id']) || !isnum($_GET['weblink_cat_id'])) { $_GET['weblink_cat_id'] = 0; }
		if ($data['weblink_cat_id'] == $_GET['weblink_cat_id']) { $p_img = "off"; $div = ""; } else { $p_img = "on"; $div = "style='display:none'"; }
		$sublist .= "<tr>\n";
		$sublist .= "<td class='tbl2'>--".$data['weblink_cat_name']." (Language: ".$data['weblink_cat_language'].")</td>\n";
		$sublist .= "<td class='tbl2' align='right'><img src='".get_image("panel_$p_img")."' alt='' name='b_".$flipbox_id.$data['weblink_cat_id']."' onclick=\"javascript:flipBox('".$flipbox_id.$data['weblink_cat_id']."')\" /></td>\n";
		$sublist .= "</tr>\n";
		$result2 = dbquery("SELECT weblink_id, weblink_name, weblink_url
		FROM ".DB_WEBLINKS." WHERE weblink_cat='".$data['weblink_cat_id']."' ORDER BY weblink_name");
		if (dbrows($result2)) {
			$sublist .= "<tr>\n<td colspan='2'>\n";
			$sublist .= "<div id='box_".$flipbox_id.$data['weblink_cat_id']."'".$div.">\n";
			...
			$sublist .= "<tr>\n<td colspan='2'>\n";
			$sublist .= "<div id='box_".$flipbox_id.$data['weblink_cat_id']."' style='display:none'>\n";
			$sublist .= "<table width='100%' cellspacing='0' cellpadding='0'>\n<tr>\n";
			$sublist .= "<td class='tbl'>".$locale['535']."</td>\n";
			$sublist .= "</tr>\n</table>\n</div>\n";
		}
			
			$sublist .= subcats($data['weblink_cat_id']); // Pimped: changed
		}
		$sublist .= "</tr>\n";
