...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['blacklist_id']) && isnum($_GET['blacklist_id']))) {
	...
	log_admin_action("admin-2", "admin_blacklist_removed"); // Log Admin's Action
	...
	if (isset($_POST['blacklist_user'])) {
		...
		if ($blacklist_ip || $blacklist_email) {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['blacklist_id']) && isnum($_GET['blacklist_id']))) {
				log_admin_action("admin-2", "admin_blacklist_changed"); // Log Admin's Action
				...
				log_admin_action("admin-2", "admin_blacklist_added"); // Log Admin's Action
				$result = dbquery("INSERT INTO ".DB_BLACKLIST." (blacklist_ip, blacklist_user_id, blacklist_email, blacklist_reason, blacklist_datestamp) 
				VALUES ('$blacklist_ip', '".$userdata['user_id']."', '$blacklist_email', '$blacklist_reason', '".time()."')");
			}
		}
		redirect(FUSION_SELF.$aidlink);
	}
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['blacklist_id']) && isnum($_GET['blacklist_id']))) {
		$result = dbquery("SELECT blacklist_id, blacklist_ip, blacklist_email, blacklist_reason
		FROM ".DB_BLACKLIST." WHERE blacklist_id='".(int)$_GET['blacklist_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
	...
	if ($rows != 0) {
		$result = dbquery(
			"SELECT b.blacklist_id, b.blacklist_ip, b.blacklist_email, b.blacklist_reason, b.blacklist_datestamp,
		...
		while ($data = dbarray($result)) {
			...
			echo "</td>\n<td class='$row_color'>";
			echo (isset($data['user_name']) ? profile_link($data['user_id'], $data['user_name'], $data['user_status']) : $locale['466'])."</td>\n";
	...
	closetable();
	if (($rows) > 20) { echo "<div align='center' style=';margin-top:5px;'>\n".pagination(true,$_GET['rowstart'],20,$rows,3,FUSION_SELF.$aidlink."&amp;")."\n</div>\n"; }
}
