...
if (isset($_GET['status']) && !isset($message)) {
	if ($_GET['status'] == "sn") {
		...
		if ($_GET['error'] == 1) { $message .= "No Poll title entered</span>"; }
	}
...
if (isset($_POST['save'])) {
	...
	if (isset($_GET['poll_id']) && isnum($_GET['poll_id'])) {
		if ($poll_title && count($poll_option)) {
			...
			log_admin_action("admin-1", "admin_poll_edited", "", "", $poll_title." (ID: ".(int)$_GET['poll_id'].")");
		...
		if ($poll_title && count($poll_option)) {
			...
			$id = mysql_insert_id();
			log_admin_action("admin-1", "admin_poll_added", "", "", $poll_title." (ID: ".(int)$id.")");
			redirect(FUSION_SELF.$aidlink."&amp;status=sn");
		} else {
			redirect(FUSION_SELF.$aidlink."&amp;status=fail&error=1");
		}
	}
} elseif (isset($_POST['delete']) && (isset($_POST['poll_id']) && isnum($_POST['poll_id']))) {
	$result = dbquery("SELECT poll_title FROM ".DB_POLLS." WHERE poll_id='".(int)$_POST['poll_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
			$result = dbquery("DELETE FROM ".DB_POLLS." WHERE poll_id='".$_POST['poll_id']."'");
			log_admin_action("admin-1", "admin_poll_deleted", "", "", $data['poll_title']." (ID: ".(int)$_POST['poll_id'].")");
		} // todo: else.. return error msg
	...
	if (isset($_POST['edit']) && (isset($_POST['poll_id']) && isnum($_POST['poll_id']))) {
		...
		$data = dbarray(dbquery("SELECT poll_title, poll_started, poll_ended, 
		poll_opt_0, poll_opt_1, poll_opt_2, poll_opt_3, poll_opt_4, poll_opt_5, poll_opt_6, poll_opt_7, poll_opt_8, poll_opt_9
		FROM ".DB_POLLS." WHERE poll_id='".$_POST['poll_id']."'"));
