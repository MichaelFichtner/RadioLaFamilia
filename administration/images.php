...
if (isset($_GET['del'])) {
	$img = stripinput($_GET['del']);
	unlink($afolder.$img);
	log_admin_action("admin-1", "admin_image_deleted", "", "", $img);
	if ($settings['wysiwyg_enabled'] == "tinymce") { include INCLUDES."buildlist.php"; }
	...
	if (!in_array($imgext, $image_types)) {
		...
		log_admin_action("admin-1", "admin_image_added", "", "", $imgname);
		if ($settings['wysiwyg_enabled'] == "tinymce") { include INCLUDES."buildlist.php"; }
