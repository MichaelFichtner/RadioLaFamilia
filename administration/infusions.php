...
if (isset($_GET['error']) && isnum($_GET['error']) && !isset($message)) {
	if ($_GET['error'] == 0) {
		$message = "Infusion installed";
	} elseif ($_GET['error'] == 1 && isset($_GET['right'])) {
		$message = "Installation of the Infusion failed! Admin right ".$_GET['right']." already in use";
	} elseif($_GET['error'] == 1) {
		$message = "Installation of the Infusion failed! Admin right already in use";
	}
	if (isset($message)) {
...
if (isset($_POST['infuse']) && isset($_POST['infusion'])) {
			...
			if ($inf_version > $data['inf_version']) {
				if (isset($inf_altertable) && is_array($inf_altertable) && count($inf_altertable)) {
					for ($i = 1; $i < (count($inf_altertable) + 1); $i++) {
						$result1 = dbquery("ALTER TABLE ".$inf_altertable[$i]);
					}
				}
			...
			if (isset($inf_adminpanel) && is_array($inf_adminpanel) && count($inf_adminpanel)) {
				for ($i = 1; $i < (count($inf_adminpanel) + 1); $i++) {
					...
					if (!dbcount("(admin_id)", DB_ADMIN, "admin_rights='".$inf_adminpanel[$i]['rights']."'")) {
						...
						$result = dbquery("SELECT user_id, user_rights FROM ".DB_USERS." WHERE user_level='".nSUPERADMIN."'");
						while ($data = dbarray($result)) {
							$result2 = dbquery("UPDATE ".DB_USERS." SET user_rights='".$data['user_rights'].".".$inf_adminpanel[$i]['rights']."' WHERE user_id='".$data['user_id']."'");
						}
					} else {
						$error = 1;
						$error_right .= ($error_right == "" ? "" : ", ").$inf_adminpanel[$i]['rights']; // Pimped
					}
				}
			}
			if (!$error) {
	...
	log_admin_action("admin-3", "admin_infusion_installed", "", "", $inf_title);
	redirect(FUSION_SELF.$aidlink."&amp;error=".$error.( $error_right == "" ? "" : "&amp;right=".$error_right));
}


if (isset($_GET['defuse']) && isnum($_GET['defuse'])) {
	$result = dbquery("SELECT inf_title, inf_folder FROM ".DB_INFUSIONS." WHERE inf_id='".$_GET['defuse']."'");
	$data = dbarray($result);
	...
	if (isset($inf_adminpanel) && is_array($inf_adminpanel) && count($inf_adminpanel)) {
		for ($i = 1; $i < (count($inf_adminpanel) + 1); $i++) {
			...
			$result = dbquery("SELECT user_id, user_rights FROM ".DB_USERS." WHERE user_level>='".nADMIN."'");
			while ($data = dbarray($result)) {
	...
	log_admin_action("admin-3", "admin_infusion_deleted", "", "", $inf_title);
	redirect(FUSION_SELF.$aidlink);
}

$result = dbquery("SELECT inf_id, inf_folder, inf_title, inf_version FROM ".DB_INFUSIONS." ORDER BY inf_title");
if (dbrows($result)) {
	$i = 0;
	...
	while ($data = dbarray($result)) {
		...
		// Pimped: auto delete not existing infusions
			$result_auto_del = dbquery("DELETE FROM ".DB_INFUSIONS." WHERE inf_folder='".$data['inf_folder']."'");
			if($result_auto_del) {
				$result_2 = dbquery("DELETE FROM ".DB_ADMIN." WHERE admin_link LIKE '%".INFUSIONS.$data['inf_folder']."%' AND admin_page='5'");
				log_admin_action("admin-3", "admin_infusion_autodeleted"); // Log Admin's Action
				$autodeleted_infusions .= $data['inf_folder']."<br />";
			}
		}
		$inf_title = ""; $inf_description = ""; $inf_version = ""; $inf_developer = ""; $inf_email = ""; $inf_weburl = "";
		$inf_folder = ""; $inf_newtable = ""; $inf_insertdbrow = ""; $inf_droptable = ""; $inf_altertable = "";
	...
	if($autodeleted_infusions != "") { // Pimped
		opentable("");
		echo "The following infusions were automatically deleted, because they could not be found on the server:<br />";
		echo $autodeleted_infusions;
		closetable();
	}
	
} else { // Pimped
	opentable($locale['404']);
	echo "<br /><div>There are no infusions installed!</div><br /><br />";
	closetable();
}
