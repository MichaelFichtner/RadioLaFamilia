...
if (isset($_GET['status']) && !isset($message)) {
	...
	} elseif($_GET['status'] == "sau") {
		$message = $locale['408'];
	} elseif($_GET['status'] == "ngn") {
		$message = "No Group Name entered";
	}
	if (isset($message)) { echo "<div id='close-message'><div class='admin-message'>".$message."</div></div>\n"; }
}

if (iSUPERADMIN && isset($_POST['update_admin']) && (isset($_GET['group_id']) && isnum($_GET['group_id']))) {
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		if (isset($_POST['rights'])) {
			$group_rights = "";
			for ($i = 0;$i < count($_POST['rights']);$i++) {
				$group_rights .= ($group_rights != "" ? "." : "").stripinput($_POST['rights'][$i]);
			}
			$result = dbquery("UPDATE ".DB_USER_GROUPS." SET group_rights='$group_rights' WHERE group_id='".$_GET['group_id']."'");
		} else {
			$result = dbquery("UPDATE ".DB_USER_GROUPS." SET group_rights='' WHERE group_id='".$_GET['group_id']."'");
		}
		log_admin_action("admin-2", "admin_groupadminrights_changed", "", "", $_GET['group_id']); // Log Admin's Action
		set_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "");
		redirect(FUSION_SELF.$aidlink."&status=sau");
	} else {
	...
	if ($group_name) {
		if (isset($_GET['group_id']) && isnum($_GET['group_id'])) {
			...
			$result = dbquery("INSERT INTO ".DB_USER_GROUPS." (group_name, group_description, group_rights) VALUES ('".$group_name."', '".$group_description."', '')");
			redirect(FUSION_SELF.$aidlink."&status=sn");
		}
	} else {
		redirect(FUSION_SELF.$aidlink."&status=ngn");
	}
	...
	if ($check_count > 0) {
		$result = dbquery("SELECT user_id, user_groups FROM ".DB_USERS." WHERE user_id IN($user_ids)");
	...
	if ($check_count > 0) {
		$result = dbquery("SELECT user_id, user_groups FROM ".DB_USERS." WHERE user_id IN($user_ids) AND user_groups REGEXP('^\\\.{$_GET['group_id']}$|\\\.{$_GET['group_id']}\\\.|\\\.{$_GET['group_id']}$')");
		while ($data = dbarray($result)) {
	...
	$result = dbquery("SELECT user_id, user_groups FROM ".DB_USERS." WHERE user_groups REGEXP('^\\\.{$_GET['group_id']}$|\\\.{$_GET['group_id']}\\\.|\\\.{$_GET['group_id']}$')");
	while ($data = dbarray($result)) {
	...
	if (isset($_GET['group_id']) && isnum($_GET['group_id'])) {
	    ...
	    $_GET['rowstart'] = $_GET['page'] > 0 ? ($_GET['page']-1) * 20 : "0"; // Pimped
		if ($rows) {
			$i = 0;
		...
		echo "</table>\n</form>\n";
		if ($rows > 20) { echo "<div align='center' style='margin-top:5px;'>\n".pagination(true,$_GET['rowstart'],20,$rows,3,FUSION_SELF.$aidlink."&amp;group_id=".$_GET['group_id']."&amp;")."\n</div>\n"; } // Pimped
		closetable();
		
		// Assign Admin Rights to the Group
	if(iSUPERADMIN) {
		$result = dbquery("SELECT group_rights FROM ".DB_USER_GROUPS." WHERE group_id='".$_GET['group_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
			...
			$admin_page = array($locale['471'], $locale['472'], $locale['473'], $locale['474'], $locale['475']);
			echo "<form name='rightsform' method='post' action='".FUSION_SELF.$aidlink."&amp;group_id=".$_GET['group_id']."'>\n";
			echo "<table cellpadding='0' cellspacing='1' width='450' class='tbl-border center'>\n";
			...
			while ($data2 = dbarray($result2)) {
				...
				echo "<td width='50%' class='tbl1'><label><input type='checkbox' name='rights[]' value='".$data2['admin_rights']."'".(in_array($data2['admin_rights'], $group_rights) ? " checked='checked'" : "")." />";
					if(array_key_exists("admin_".$data2['admin_rights'], $locale) && ($page == 1 || $page == 2 || $page == 3 || $page == 4 )) { // PiF: Locale
						echo $locale["admin_".$data2['admin_rights']];
					} else {
			...
			echo "</tr>\n<tr>\n</table>\n";
			echo "<div style='text-align:center'><br />\n";
			echo "<input type='button' class='button' onclick=\"setChecked('rightsform','rights[]',1);\" value='".$locale['448']."' />\n";
			echo "<input type='button' class='button' onclick=\"setChecked('rightsform','rights[]',0);\" value='".$locale['449']."' /><br /><br />\n";
			if (!check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
				echo $locale['480']." <input type='password' name='admin_password' class='textbox' style='width:150px;' /><br /><br />\n";
			}
			echo "<input type='submit' name='update_admin' value='".$locale['481']."' class='button' />\n";
			echo "</div>\n</form>\n";
			closetable();
