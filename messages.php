...
$msg_settings = dbarray(dbquery("SELECT pm_email_notify, pm_save_sent, pm_inbox, pm_savebox, pm_sentbox  FROM ".DB_MESSAGES_OPTIONS." WHERE user_id='0'")); // Pimped: Mysql-Query
...
if (isset($_POST['send_message'])) {
	$result = dbquery("SELECT pm_save_sent, pm_email_notify FROM ".DB_MESSAGES_OPTIONS." WHERE user_id='".$userdata['user_id']."'"); // Pimped: Mysql-Query
	if (dbrows($result)) {
	...
	if (iADMIN && isset($_POST['chk_sendtoall']) && isnum($_POST['msg_to_group'])) {
		...
		if ($msg_to_group == nMEMBER || $msg_to_group == nMODERATOR || $msg_to_group == nADMIN || $msg_to_group == nSUPERADMIN) { // Pimped
			$result = dbquery(
		...
		if (!flood_control("message_datestamp", DB_MESSAGES, "message_from='".$userdata['user_id']."'")) {
			$result = dbquery(
				"SELECT u.user_id, u.user_name, u.user_email, u.user_level, u.user_language, mo.pm_email_notify, s.pm_inbox, COUNT(message_id) as message_count
				FROM ".DB_USERS." u
				LEFT JOIN ".DB_MESSAGES_OPTIONS." mo USING(user_id)
			...
			if (dbrows($result)) {
				$data = dbarray($result);
				if ($data['user_id'] != $userdata['user_id']) {
					if ($data['user_id'] == 1 || $data['user_level'] > nMEMBER  || $data['pm_inbox'] == "0" || ($data['message_count'] + 1) <= $data['pm_inbox']) { // Pimped
						...
						$send_email = isset($data['pm_email_notify']) ? $data['pm_email_notify'] : $msg_settings['pm_email_notify'];
						if ($send_email == "1") {
							if($data['user_language'] != "" && $data['user_language'] != $settings['locale'] && 
							file_exists(LOCALE.$data['user_language']."/messages.php")) {
								include LOCALE.$data['user_language']."/messages.php";
								$reinc = true;
							} else {
								$reinc = false;
							}
							...
							sendemail($data['user_name'], $data['user_email'], $settings['siteusername'], $settings['siteemail'],
								$locale['625'], $data['user_name'].$message_content);
							if($reinc) include LOCALE.LOCALESET."messages.php";
						}
					} else {
						$error = "2";
					}
...
if (!isset($_GET['msg_send']) && !isset($_GET['msg_read']) && $_GET['folder'] != "options") {
	...
	echo "<td width='1%' class='tbl' style='white-space:nowrap;font-weight:".($_GET['folder']=="outbox"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=outbox'>".$locale['403']." [".$bdata['outbox_total']."/".($msg_settings['pm_sentbox'] != 0 ? $msg_settings['pm_sentbox'] : "&infin;")."]</a></td>\n";
	echo "<td width='1%' class='tbl' style='white-space:nowrap;font-weight:".($_GET['folder']=="archive"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=archive'>".$locale['404']." [".$bdata['archive_total']."/".($msg_settings['pm_savebox'] != 0 ? $msg_settings['pm_savebox'] : "&infin;")."]</a></td>\n";
	...
	echo "}\n}\n}\n</script>\n";
	closetable();
	if ($total_rows > 20) echo "<div align='center' style='margin-top:5px;'>\n".pagination(true,$_GET['rowstart'], 20, $total_rows, 3, FUSION_SELF."?folder=".$_GET['folder']."&amp;")."\n</div>\n"; // Pimped
	...
	$result = dbquery(
		"SELECT m.message_id, message_subject, message_message, message_smileys, message_datestamp, message_folder,
		u.user_id, u.user_name, u.user_status
		FROM ".DB_MESSAGES." m
		LEFT JOIN ".DB_USERS." u ON m.message_from=u.user_id
	...
	if (isset($_GET['msg_id']) && isnum($_GET['msg_id'])) {
		$result = dbquery(
			"SELECT m.message_subject, m.message_message, m.message_smileys,
		...
		if($message == "") $message = "[quote][b]".$data['user_name']." wrote:[/b]\n".$data['message_message']."\n[/quote]"; // Pimped. todo localize
	...
	if (!isset($_POST['chk_sendtoall']) || $_GET['msg_send'] != "0") {
		...
		$result = dbquery("SELECT u.user_id, u.user_name FROM ".DB_USERS." u WHERE user_status='0' ORDER BY user_level DESC, user_name ASC");
		while ($data = dbarray($result)) {
			if ($data['user_id'] != $userdata['user_id']) {
	...
	echo "<tr>\n<td class='tbl'>";
	echo "<a href='".FUSION_SELF."?folder=inbox' style='text-decoration:none;'><input type='button' value='".$locale['435']."' class='button'></a>";
	echo "</td>\n";
