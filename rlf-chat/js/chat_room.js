      ...
      if (typeof(converted_parts[i])=='undefined') {
        ...
        if (msg_parts[i]!='') {
          for (var code in SmilieList.SmilieList) {
            if (msg_parts[i]==code) {
              msg_parts[i]='<img src="'+SmilieList.SmilieList[code].source+'" alt="'+code+'" title="'+code+'" onclick="$(\'main_input_textarea\').value = $(\'main_input_textarea\').value + \' '+code+' \'; $(\'main_input_textarea\').focus();" />';
...
function openOnlineStatusBox(openerObj) {
  ...
  if (online_status_selection_box.style.display=='none') {
	...
	$('online_status_4_pointer').src='./pic/clearpixel_1x1.gif';
...
var xmlHttp;
   ...
   if (xmlHttp) {
      xmlHttp.open('GET', './mods/trans.php', true);
      xmlHttp.onreadystatechange = trans_handler;
      xmlHttp.send(null);
   }
   
   function trans_handler() {
      if (xmlHttp.readyState == 4) {
         $('trans').innerHTML=xmlHttp.responseText;
         $('trans1').value=xmlHttp.responseText;
                if($('trans0').value!=$('trans1').value){
                      displayMessage(null, ' >>> '+htmlspecialchars(xmlHttp.responseText), null, false);
                      $('trans0').value=xmlHttp.responseText;
                }
      }
   }
   window.setTimeout("trans()", 2000);
}
