...
if ($_GET['stype'] == "l") {
	if (isset($_POST['submit_link'])) {
		if ($_POST['link_name'] != "" && $_POST['link_url'] != "" && $_POST['link_description'] != "") {
			...
			echo "<a href='".make_url("submit.php?stype=l", "submit-link", "", ".html")."'>".$locale['411']."</a><br /><br />\n"; // Pimped: make_url
		...
		function editlist_sub($id) {
			$result = dbquery("SELECT weblink_cat_id, weblink_cat_name
			FROM ".DB_WEBLINK_CATS." WHERE ".groupaccess("weblink_cat_access")." AND weblink_cat_parent='".$id."' ORDER BY weblink_cat_name");
			$catlist = "";
			$k = dbrows($result);
			if ($k > 0) {
				while ($data = dbarray($result)) {
					$catlist .= "<option value='".$data['weblink_cat_id']."'>--".$data['weblink_cat_name']."</option>\n";
				}
			}
			return $catlist;	
		}
		$opts = "";
		$result2 = dbquery("SELECT weblink_cat_id, weblink_cat_name
		FROM ".DB_WEBLINK_CATS." WHERE ".groupaccess("weblink_cat_access")." AND weblink_cat_parent='0' ORDER BY weblink_cat_name");
		if (dbrows($result2) != 0) {
			while ($data2 = dbarray($result2)) {
				$opts .= "<option value='".$data2['weblink_cat_id']."'>".$data2['weblink_cat_name']."</option>\n"; //pimped
				$opts .= editlist_sub($data2['weblink_cat_id']);//pimped
			}
		}
		if ($opts != "") {
			echo $locale['420']."<br /><br />\n";
			echo "<form name='submit_form' method='post' action='".make_url("submit.php?stype=l", "submit-link", "", ".html")."' onsubmit='return validateLink(this);'>\n"; // Pimped: make_url
	...
	if (isset($_POST['submit_news'])) {
		if ($_POST['news_subject'] != "" && $_POST['news_body'] != "") {
			...
			echo "<div style='text-align:center'><br />\n".$locale['460']."<br /><br />\n";
			echo "<a href='".make_url("submit.php?stype=n", "submit-news", "", ".html")."'>".$locale['461']."</a><br /><br />\n"; // Pimped: make_url
			echo "<a href='index.php'>".$locale['412']."</a><br /><br />\n</div>\n";
			closetable();
		}
	} else {
		...
		echo "<form name='submit_form' method='post' action='".make_url("submit.php?stype=n", "submit-news", "", ".html")."' onsubmit='return validateNews(this);'>\n"; // Pimped: url-rewrite
		echo "<table cellpadding='0' cellspacing='0' class='center'>\n<tr>\n";
		...
		echo "<td class='tbl'><textarea name='news_body' cols='60' rows='12' class='textbox' style='width:300px;'>$news_body</textarea></td>\n";
		echo "</tr>\n<tr>\n";
		echo "<td align='center' colspan='2' class='tbl'><br />\n";
	...
	if (isset($_POST['submit_article'])) {
		if ($_POST['article_subject'] != "" && $_POST['article_body'] != "") {
			...
			echo "<div style='text-align:center'><br />\n".$locale['510']."<br /><br />\n";
			echo "<a href='".make_url("submit.php?stype=a", "submit-article", "", ".html")."'>".$locale['511']."</a><br /><br />\n"; // Pimped: make_url
			echo "<a href='index.php'>".$locale['412']."</a><br /><br />\n</div>\n";
			closetable();
		}
	} else {
		...
		function a_editlist_sub($id, $edited_id) {
			$result = dbquery("SELECT article_cat_id, article_cat_name, article_cat_language
			FROM ".DB_ARTICLE_CATS." WHERE ".groupaccess("article_cat_access")." AND article_cat_parent='".(int)$id."' ORDER BY article_cat_name");
			$catlist = ""; $sel = "";
			$k = dbrows($result);
			if ($k > 0) {
				while ($data = dbarray($result)) {
					if (isset($_POST['preview_article'])) { $sel = ($edited_id == $data['article_cat_id'] ? " selected='selected'" : ""); }
		...
		$result = dbquery("SELECT article_cat_id, article_cat_name, article_cat_language
		FROM ".DB_ARTICLE_CATS." WHERE ".groupaccess("article_cat_access")." AND article_cat_parent='0' ORDER BY article_cat_name");//pimped
		if (isset($_POST['preview_article'])) { $edited_id = $article_cat; } else { $edited_id = ""; }
		$sel = "";
		while ($data = dbarray($result)) {
			if (isset($_POST['preview_article'])) $sel = ($article_cat == $data['article_cat_id'] ? " selected='selected'" : "");
			$cat_list .= "<option value='".$data['article_cat_id']."'".$sel.">".$data['article_cat_name']."</option>\n"; // Pimped
			$cat_list .= a_editlist_sub($data['article_cat_id'], $edited_id); // Pimped
		}
		
		if ($cat_list != "") {
			echo $locale['520']."<br /><br />\n";
			echo "<form name='submit_form' method='post' action='".make_url("submit.php?stype=a", "submit-article", "", ".html")."' onsubmit='return validateArticle(this);'>\n"; // Pimped: make_url
			echo "<table cellpadding='0' cellspacing='0' class='center'>\n<tr>\n";
			...
			echo "<td class='tbl'><textarea name='article_snippet' cols='60' rows='5' class='textbox' style='width:300px;'>$article_snippet</textarea></td>\n";
			echo "</tr>\n<tr>\n";
			echo "<td valign='top' class='tbl'>".$locale['524']."</td>\n";
			echo "<td class='tbl'><textarea name='article_body' cols='60' rows='15' class='textbox' style='width:300px;'>$article_body</textarea></td>\n";
			echo "</tr>\n<tr>\n";
			echo "<td align='center' colspan='2' class='tbl'><br />\n";
	...
	if (isset($_POST['submit_photo'])) {
		...
		if (!$error) {
			...
			echo "<div style='text-align:center'><br />\n".$locale['580']."<br /><br />\n";
			echo "<a href='".make_url("submit.php?stype=p", "submit-photo", "", ".html")."'>".$locale['581']."</a><br /><br />\n"; // Pimped: make_url
			echo "<a href='".make_url("index.php", "index", "", ".html")."'>".$locale['412']."</a><br /><br />\n</div>\n"; // Pimped: make_url
		} else {
			echo "<div style='text-align:center'><br />\n".$locale['600']."<br /><br />\n";
			...
			echo "<br /><br />\n<a href='".make_url("submit.php?stype=p", "submit-photo", "", ".html")."'>".$locale['581']."</a><br /><br />\n</div>\n"; // Pimped: make_url
		}
		closetable();
	} else {
		$opts = "";
		...
		if (dbrows($result)) {
			...
			echo "<form name='submit_form' method='post' action='".make_url("submit.php?stype=p", "submit-photo", "", ".html")."' enctype='multipart/form-data' onsubmit='return validatePhoto(this);'>\n"; // Pimped: url-rewrite
			echo "<table cellpadding='0' cellspacing='0' class='center'>\n<tr>\n";
...
function validateLink(frm) {
	if (frm.link_name.value==\"\" || frm.link_url.value==\"\" || frm.link_url.value==\"http://\" || frm.link_description.value==\"\") {
