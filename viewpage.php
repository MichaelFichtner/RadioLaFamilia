...
$cp_result = dbquery("SELECT page_title, page_access, page_content, page_keywords, page_allow_comments, page_allow_ratings
		FROM ".DB_CUSTOM_PAGES." WHERE page_id='".(int)$_GET['page_id']."'"); // Pimped
if (dbrows($cp_result)) {
	$cp_data = dbarray($cp_result);
	add_to_title($locale['global_200'].$cp_data['page_title']);
	set_meta('keywords', $cp_data['page_keywords'], false);  // meta
	set_meta('description', trimlink($cp_data['page_title'], 255), false);  // meta
	...
	if (checkgroup($cp_data['page_access'])) {
		...
		echo itemoptions("CP", (int)$_GET['page_id']); // Pimped
	} else {
...
if (dbrows($cp_result) && checkgroup($cp_data['page_access'])) {
	if ($settings['enable_tags']) {
			require_once INCLUDES."tag_include.php"; // Pimped: tag
			echo show_tags((int)$_GET['page_id'], "C"); // Pimped: tag
	}
