...
if (isset($_GET['photo_id']) && isnum($_GET['photo_id'])){
	...
	tp.photo_order, tp.photo_allow_comments, tp.photo_allow_ratings,
	ta.album_id, ta.album_title, ta.album_access,
	...
	WHERE photo_id='".(int)$_GET['photo_id']."' GROUP BY tp.photo_id");
	$data = dbarray($result);
	if (!checkgroup($data['album_access'])) {
		...
		$result = dbquery("UPDATE ".DB_PHOTOS." SET photo_views=(photo_views+1) WHERE photo_id='".(int)$_GET['photo_id']."'");

		$pres = dbquery("SELECT photo_id, photo_title FROM ".DB_PHOTOS." WHERE photo_order='".($data['photo_order']-1)."' AND album_id='".$data['album_id']."'");
		$nres = dbquery("SELECT photo_id, photo_title  FROM ".DB_PHOTOS." WHERE photo_order='".($data['photo_order']+1)."' AND album_id='".$data['album_id']."'");
		...
		if ($settings['photo_watermark']) {
			if ($settings['photo_watermark_save']) {
				...
				if (!file_exists(PHOTODIR.$wm_file1)) {
					if ($data['photo_thumb2']) { $photo_thumb = "photo.php?photo_id=".(int)$_GET['photo_id']; }
					$photo_file = "photo.php?photo_id=".(int)$_GET['photo_id'];
				} else {
		...
		echo "<a href='".make_url(BASEDIR.FUSION_SELF, BASEDIR."photogallery", "", ".html")."'>".$locale['400']."</a> &gt;\n";
		echo "<a href='".make_url(BASEDIR.FUSION_SELF."?album_id=".$data['album_id'], BASEDIR."photogallery-album-".$data['album_id']."-", $data['album_title'], ".html")."'>".$data['album_title']."</a> &gt;\n";
		echo "<a href='".make_url(BASEDIR.FUSION_SELF."?photo_id=".$_GET['photo_id'], BASEDIR."photo-".$_GET['photo_id']."-", $data['photo_title'], ".html")."'>".$data['photo_title']."</a>\n</td>\n";
		if ((isset($prev['photo_id']) && isnum($prev['photo_id'])) || (isset($next['photo_id']) && isnum($next['photo_id']))) {
			if (isset($prev)) { echo "<td width='1%' class='tbl2'><a href='".make_url(BASEDIR.FUSION_SELF."?photo_id=".$prev['photo_id'], BASEDIR."photo-".$prev['photo_id']."-", $prev['photo_title'], ".html")."' title='".$locale['451']."'>&lt;&lt;</a></td>\n"; }
			if (isset($next)) { echo "<td width='1%' class='tbl2'><a href='".make_url(BASEDIR.FUSION_SELF."?photo_id=".$next['photo_id'], BASEDIR."photo-".$next['photo_id']."-", $next['photo_title'], ".html")."' title='".$locale['452']."'>&gt;&gt;</a></td>\n"; }
		}
		echo "</tr>\n</table>\n";

		echo "<div align='center' style='margin:5px;'>\n";
		if($settings['enable_lightbox'] == "1") { // Pimped: Lightbox
	...
	opentable($locale['407']);
		$result = dbquery(
			"SELECT ta.album_id, album_title, album_thumb, album_datestamp, tu.user_id, tu.user_name, tu.user_status FROM ".DB_PHOTO_ALBUMS." ta
			LEFT JOIN ".DB_USERS." tu ON ta.album_user=tu.user_id
			WHERE ".groupaccess('album_access')."AND album_parent='".$_GET['album_id']."' ORDER BY album_order
	...
	//Pimped subcategories end
			if ($rows) {
				...
				if (!isset($_GET['page']) || !isnum($_GET['page'])) { $_GET['page'] = 0; }
			    $_GET['rowstart'] = $_GET['page'] > 0 ? ($_GET['page']-1) * $settings['thumbs_per_page'] : "0";
				$result = dbquery("SELECT tp.photo_id, tp.photo_title, tp.photo_thumb1, tp.photo_datestamp, tp.photo_views,
					tu.user_id, tu.user_name, tu.user_status, SUM(tr.rating_vote) AS sum_rating, COUNT(tr.rating_item_id) AS count_votes
					FROM ".DB_PHOTOS." tp
					LEFT JOIN ".DB_USERS." tu ON tp.photo_user=tu.user_id
					LEFT JOIN ".DB_RATINGS." tr ON tr.rating_item_id = tp.photo_id AND tr.rating_type='P'
					WHERE album_id='".(int)$_GET['album_id']."' GROUP BY photo_id ORDER BY photo_order LIMIT ".(int)$_GET['rowstart'].",".(int)$settings['thumbs_per_page']);
				$counter = 0;
				echo "<table cellpadding='0' cellspacing='1' width='100%'>\n<tr>\n<td class='tbl2'>\n";
				echo "<a href='".make_url(BASEDIR.FUSION_SELF, BASEDIR."photogallery", "", ".html")."'>".$locale['400']."</a> &gt;\n";
				echo "<a href='".make_url(BASEDIR.FUSION_SELF."?album_id=".(int)$_GET['album_id'], BASEDIR."photogallery-album-".(int)$_GET['album_id']."-", $data['album_title'], ".html")."'>".$data['album_title']."</a>\n";
				echo "</td>\n</tr>\n</table>\n";
				if ($rows > $settings['thumbs_per_page']) { echo "<div align='center' style='margin-top:5px;'>\n".pagination(true,$_GET['rowstart'], $settings['thumbs_per_page'], $rows, 3, FUSION_SELF."?album_id=".$_GET['album_id']."&amp;", "photogallery-album", "-",$_GET['album_id'],"-", "-", $data['album_title'])."\n</div>\n"; }
				echo "<table cellpadding='0' cellspacing='1' width='100%'>\n<tr>\n";
				while ($data2 = dbarray($result)) { // Pimped: $data2 instead of $data
					if ($counter != 0 && ($counter % $settings['thumbs_per_row'] == 0)) { echo "</tr>\n<tr>\n"; }
					echo "<td align='center' valign='top' class='tbl'>\n";
					echo "<strong>".$data2['photo_title']."</strong><br /><br />\n<a href='".make_url(BASEDIR.FUSION_SELF."?photo_id=".$data2['photo_id'], BASEDIR."photo-".$data2['photo_id']."-", $data2['photo_title'], ".html")."' class='photogallery_album_photo_link'><!--photogallery_album_photo_".$data2['photo_id']."-->";
					if ($data2['photo_thumb1'] && file_exists(PHOTODIR.$data2['photo_thumb1'])){
						echo "<img src='".PHOTODIR.$data2['photo_thumb1']."' alt='".$data2['photo_thumb1']."' title='".$locale['431']."' style='border:0px' class='photogallery_album_photo' />";
					} elseif(file_exists(PHOTOS."nophoto.jpg")) {
						echo "<img src='".PHOTOS."nophoto.jpg' alt='".$locale['432']."' title='".$locale['432']."' style='border:0px' />";
					} else {
						echo $locale['432'];
					...
					echo $locale['433'].showdate("shortdate", $data2['photo_datestamp'])."<br />\n";
					echo $locale['434'].profile_link($data2['user_id'], $data2['user_name'], $data2['user_status'])."<br />\n";
					$photo_comments = dbcount("(comment_id)", DB_COMMENTS, "comment_type='P' AND comment_item_id='".(int)$data2['photo_id']."'");
					echo ($photo_comments == 1 ? $locale['436b'] : $locale['436']).$photo_comments."<br />\n";
					echo $locale['437'].($data2['count_votes'] > 0 ? str_repeat("<img src='".get_image("star")."' alt='*' style='vertical-align:middle' />", ceil($data2['sum_rating'] / $data2['count_votes'])) : $locale['438'])."<br />\n";
					echo $locale['435'].$data2['photo_views']."</span><br />\n";
					echo "</td>\n";
					$counter++;
				}
				echo "</tr>\n</table>\n";
	...
	$rows = dbcount("(album_id)", DB_PHOTO_ALBUMS, groupaccess('album_access')." AND album_parent='0'");
	if (!isset($_GET['page']) || !isnum($_GET['page'])) { $_GET['page'] = 0; }
	$_GET['rowstart'] = $_GET['page'] > 0 ? ($_GET['page']-1) * $settings['thumbs_per_page'] : "0";
	if ($rows) {
		$result = dbquery(
			"SELECT ta.album_id, album_title, album_thumb, album_datestamp,
