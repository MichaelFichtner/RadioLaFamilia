...
if (isset($_GET['delete']) && isnum($_GET['delete']) && dbcount("(thread_id)", DB_THREAD_NOTIFY, "thread_id='".(int)$_GET['delete']."' AND notify_user='".(int)$userdata['user_id']."'")) {
	$result = dbquery("DELETE FROM ".DB_THREAD_NOTIFY." WHERE thread_id=".(int)$_GET['delete']." AND notify_user=".(int)$userdata['user_id']);
...
if ($rows) {
	...
	WHERE tn.notify_user=".(int)$userdata['user_id']." AND ".groupaccess('tf.forum_access')." AND tt.thread_hidden='0'
	GROUP BY tn.thread_id
	ORDER BY tn.notify_datestamp DESC
	LIMIT ".(int)$_GET['rowstart'].",10
	");
  echo "<table class='tbl-border' cellpadding='0' cellspacing='1' width='100%'>\n<tr>\n";
	...
	if(IF_MULTI_LANGUAGE_FORUM) echo "<td class='tbl2' style='text-align:center;white-space:nowrap'><strong>".$locale['global_061']."</strong></td>\n";
	...
	while ($data = dbarray($result)) {
		...
		if(IF_MULTI_LANGUAGE_FORUM) echo "<td class='".$row_color."' style='text-align:center;white-space:nowrap'>".get_image($data['forum_language'], $data['forum_language'], "", $data['forum_language'], "", true)."</td>\n"; // Pimped
