...
if ($rows) {
	...
	$result = dbquery(
		...
		FROM ".DB_POSTS." tp
		LEFT JOIN ".DB_FORUMS." tf ON tp.forum_id = tf.forum_id
		LEFT JOIN ".DB_FORUMS." tc ON tf.forum_cat=tc.forum_id
		LEFT JOIN ".DB_THREADS." tt ON tp.thread_id = tt.thread_id
		LEFT JOIN ".DB_USERS." tu ON tp.post_author = tu.user_id
		WHERE ".groupaccess('tf.forum_access')." AND tp.post_hidden='0' AND tt.thread_hidden='0' AND tp.post_datestamp > '".$lastvisited."'".$insert." 
	...
	while ($data = dbarray($result)) {
		...
		echo "<td class='".$row_color."'><a href='".make_url(FORUM."viewthread.php?thread_id=".$data['thread_id']."&amp;pid=".$data['post_id'], BASEDIR."forum-thread-".$data['thread_id']."-pid".$data['post_id']."-", $data['thread_subject'], ".html")."#post_".$data['post_id']."'>".$data['thread_subject']."</a></td>\n"; // Pimped
