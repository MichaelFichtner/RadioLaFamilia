...
if (iMEMBER && isset($_POST['cast_vote']) && (isset($_POST['poll_id']) && isnum($_POST['poll_id']))) {
	$count = dbcount("(vote_id)", DB_POLL_VOTES, "vote_user='".$userdata['user_id']."' AND poll_id='".$_POST['poll_id']."'");
	if (!$count) {
...
$result = dbquery("SELECT
					...
					poll_started, poll_ended
					FROM ".DB_POLLS." ORDER BY poll_started DESC LIMIT 1");
if (dbrows($result)) {
	$data = dbarray($result);
	...
	if (iMEMBER) $result2 = dbquery("SELECT vote_id FROM ".DB_POLL_VOTES." WHERE vote_user='".$userdata['user_id']."' AND poll_id='".$data['poll_id']."'");
