...
$result = dbquery("SELECT poll_id, poll_title FROM ".DB_POLLS." WHERE poll_ended!='0' ORDER BY poll_id DESC");
if (dbrows($result)) {
	$view_list = "";
...
if (isset($_POST['view']) && (isset($_POST['viewpoll_id']) && isnum($_POST['viewpoll_id']))) {
	$result = dbquery("SELECT
					...
					FROM ".DB_POLLS." WHERE poll_id='".(int)$_POST['viewpoll_id']."' AND poll_ended!='0'");
	if (dbrows($result)) {
		...
		while ($i < count($viewpoll_option)) {
			...
			if (iADMIN) {
				$result = dbquery(
					"SELECT tu.user_name FROM ".DB_POLL_VOTES." tp
