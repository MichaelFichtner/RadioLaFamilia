...
if ((isset($_GET['type']) && $_GET['type'] == "A") && (isset($_GET['item_id']) && isnum($_GET['item_id']))) {
	$result = dbquery(
		"SELECT ta.article_subject, ta.article_article, ta.article_breaks, ta.article_datestamp,
		tac.article_cat_access,
		tu.user_id, tu.user_name, tu.user_status
		FROM ".DB_ARTICLES." ta
		INNER JOIN ".DB_ARTICLE_CATS." tac ON ta.article_cat=tac.article_cat_id
		LEFT JOIN ".DB_USERS." tu ON ta.article_name=tu.user_id
		WHERE article_id='".(int)$_GET['item_id']."' AND article_draft='0'"
	);
	$res = false;
	if (dbrows($result)) {
		$data = dbarray($result);
	...
	$result = dbquery(
		...
		WHERE news_id='".(int)$_GET['item_id']."' AND news_draft='0'"
	);
	$res = false;
	if (dbrows($result) != 0) {
		$data = dbarray($result);
	...
	$rowstart = $_GET['page'] > 0 ? ($_GET['page']-1) * $settings['posts_per_page'] : "0";
	...
	WHERE ft.thread_id=".(int)$_GET['thread']."
	ORDER BY fp.post_datestamp
	LIMIT ".(int)$rowstart.",".(int)$settings['posts_per_page']);
	...
	if (dbrows($result)) {
		while ($data = dbarray($result)) {
			if (checkgroup($data['forum_access'])) {
				...
				echo parseubb(nl2br($data['post_message']), false, true);
				if ($data['edit_name']!='') {
					echo "<div style='margin-left:20px'>\n<hr />\n";
	...
	INNER JOIN ".DB_USERS." fu ON fu.user_id = fp.post_author
	LEFT JOIN ".DB_USERS." fe ON fe.user_id = fp.post_edituser
	WHERE ft.thread_id=".(int)$_GET['thread']." AND fp.post_id = ".(int)$_GET['post']);
	$res = false;
	if (dbrows($result)) {
		$data = dbarray($result);
		if (checkgroup($data['forum_access'])) {
