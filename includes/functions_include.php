...
function stripfilename($filename) {
	$filename = strtolower(str_replace(" ", "_", $filename)); // Replace whitespaces with _
	$filename = preg_replace("/[^a-zA-Z0-9_-]/", "", $filename); // Replases all non-alphanumeric chars or _-
	$filename = preg_replace("/^\W/", "", $filename); // Replace non-alphanumeric at the beginning
	$filename = preg_replace('/([_-])\1+/', '$1', $filename); // Replaces doubbles -_
	
	return $filename;
}

// Sets the value of a setting in the settings_inf table
function set_setting($setting_name, $setting_value, $setting_inf) { // Pimped
	$set_result = dbquery("SELECT settings_name FROM ".DB_SETTINGS_INF." WHERE settings_name="._db($setting_name)." AND settings_inf="._db($setting_inf));
	if (dbrows($set_result)) {
		$up_result = dbquery("UPDATE ".DB_SETTINGS_INF." SET settings_value="._db($setting_value)." WHERE settings_name="._db($setting_name)." AND settings_inf="._db($setting_inf)."");
		return ($up_result ? true : false);
	} else {
		$in_result = dbquery("INSERT INTO ".DB_SETTINGS_INF." (settings_name, settings_value, settings_inf) VALUES ("._db($setting_name).", "._db($setting_value).", "._db($setting_inf).")");
		return ($in_result ? true : false);
	}
}
...
// Send PM to a user
function send_pm($to, $from, $subject, $message, $smileys = "y") { // Pimped
	global $settings;
	...
	if ($from === "0" || !flood_control("message_datestamp", DB_MESSAGES, "message_from='".$from."'")) { // Pimped
		$result = dbquery(
		...
		if (dbrows($result)) {
			...
			if ($from === "0" || dbrows($result)) {  // Pimped
				if($from === "0") {
					$userdata['user_id'] = "0";
