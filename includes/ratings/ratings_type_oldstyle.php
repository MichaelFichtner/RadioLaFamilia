...
+----------------------------------------------------------------------------+
| Filename: includes/ratings/ratings_type_oldstyle.php
...
function showratings($rating_type, $rating_item_id, $rating_link, $seo_link_root = "", $seo_link_subject = "") { // Pimped: url-rewrite

	global $settings, $locale, $userdata;
	if(URL_REWRITE && $seo_link_root != "") { $seo_link = $seo_link_root.clean_subject_urlrewrite($seo_link_subject).".html";} // Pimped
	if ($settings['ratings_enabled'] == "1") {
		if (iMEMBER) {
			...
			if (isset($_POST['post_rating'])) {
				...
				if(URL_REWRITE && $seo_link_root != "") { redirect($seo_link); } else { redirect($rating_link); } // Pimped
