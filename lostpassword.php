...
if (isset($_GET['email']) && isset($_GET['account'])) {
	...
	if ($error == 0) {
		...
		if (dbrows($result) != 0) {
			...
			$result = dbquery("UPDATE ".DB_USERS." SET user_password='".encrypt_pw($new_pass)."' WHERE user_id='".$data['user_id']."'");
	...
	if (preg_match("/^[-0-9A-Z_\.]{1,50}@([-0-9A-Z_\.]+\.){1,50}([0-9A-Z]){2,4}$/i", $email)) {
		...
		if (dbrows($result)) {
			...
			echo "<div style='text-align:center'><br />\n".$locale['404']."<br /><br />\n<a href='".make_url(FUSION_SELF, "lostpassword", "", ".html")."'>".$locale['406']."</a><br /><br />\n</div>\n";
		}
	} else {
		echo "<div style='text-align:center'><br />\n".$locale['405']."<br /><br />\n<a href='".make_url(FUSION_SELF, "lostpassword", "", ".html")."'>".$locale['403']."</a><br /><br /></div>\n";
	}
} else {
	echo "<div style='text-align:center'>\n<form name='passwordform' method='post' action='".make_url(FUSION_SELF, "lostpassword", "", ".html")."'>\n";
