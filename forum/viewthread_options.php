...
+----------------------------------------------------------------------------+
| Filename: forum/viewthread_options.php
...
if (!defined("iMOD")  || !iMOD) { redirect(make_url(FORUM."index.php", BASEDIR."forum", "", ".html")); } // Pimped: make_url

if(isnum($_GET['thread_id'])){ //pimped
$log_result = dbquery("SELECT thread_subject FROM ".DB_THREADS." WHERE thread_id='".(int)$_GET['thread_id']."'"); // Pimped
...
if (isset($_POST['delete_posts']) && isset($_POST['delete_post']) && is_array($_POST['delete_post']) && count($_POST['delete_post'])) {
	...
	foreach ($_POST['delete_post'] as $del_post_id) {
		if (isnum($del_post_id)) { $del_posts .= ($del_posts ? "," : "").$del_post_id; $i++;
						$log_result2 = dbquery("SELECT forum_id FROM ".DB_POSTS." WHERE post_id ='".$del_post_id."'"); // pimped
						$log_data2 = dbarray($log_result2);  // pimped
						log_admin_action("forum", "delete_post", $log_data2['forum_id'], $_GET['thread_id'], $log_data['thread_subject'], "", $del_post_id); // // Pimped: Log mod's actions
		}
	}
	if ($del_posts) {
	...
	if (!$thread_count) { redirect(FORUM."viewforum.php?forum_id=".$fdata['forum_id']); } // Pimped: constant FORUM added
		...
		if (dbrows($move_result)) {
			...
			if (!isset($_POST['new_forum_id']) && !$f_post_blo) {
				...
				if (dbrows($fl_result) > 0) {
					...
					$forum_list .= "</optgroup>\n";
					echo "<form name='modopts' method='post' action='".make_url(FORUM.FUSION_SELF."?thread_id=".$fdata['thread_id']."&amp;rowstart=".$_GET['rowstart'], BASEDIR."forum-thread-".$fdata['thread_id']."-start".$_GET['rowstart']."-", $fdata['thread_subject'], ".html")."'>\n"; // Pimped: make_url
					...
					echo "<div class='admin-message'><br />\n";
					...
					if (dbrows($tl_result) > 0) {
						...
						echo "<form name='modopts' method='post' action='".FORUM.FUSION_SELF."?thread_id=".$_GET['thread_id']."&amp;rowstart=".$_GET['rowstart']."&amp;sv'>\n"; //Pimped: constant FORUM added
						echo "<table cellpadding='0' cellspacing='0' width='100%' align='center'>\n<tr>\n";
						...
						echo "<div class='admin-message'>".$locale['608']."<br /><br />\n";
				...
				log_admin_action("forum", "move_posts", $pdata['forum_id'], $_GET['thread_id'], $log_data['thread_subject'], $_POST['new_forum_id'], $move_post_id, $_POST['new_thread_id']); // Pimped: Log mod's actions
					}
				}
				if ($move_posts_add) {
	...
	if ($message != "") {
		...
		echo "<div class='admin-message'>".$message."<br /><br />\n";
