...
if (!iMEMBER || !in_array($_GET['post'], $valid_get)) { redirect(make_url(FORUM."index.php", BASEDIR."forum", "", ".html")); } // Pimped: make_url
## Pimped: seo url_rewrite
if($_GET['post'] == "reply" || $_GET['post'] == "edit") {
$seo_result = dbquery(
	"SELECT t.thread_subject, f.forum_name, f2.forum_name AS forum_cat_name
	FROM ".DB_THREADS." t
	LEFT JOIN ".DB_FORUMS." f ON t.forum_id=f.forum_id
	LEFT JOIN ".DB_FORUMS." f2 ON f.forum_cat=f2.forum_id
	WHERE t.thread_id='".(int)$_GET['thread_id']."'"
); // Pimped
if (dbrows($seo_result)) {
	$seo_data = dbarray($seo_result);
}
}
##
if (($_GET['post'] == "on" || $_GET['post'] == "off") && $settings['thread_notify']) {
	...
	if (!$output) redirect(make_url(FORUM."index.php", BASEDIR."forum", "", ".html")); // Pimped: make_url
	...
	echo "<a href='".FORUM."viewforum.php?forum_id=".$_GET['forum_id']."'>".$locale['448']."</a> ::\n";	// Pimped: constant FORUM added
	echo "<a href='".make_url(FORUM."index.php", BASEDIR."forum", "", ".html")."'>".$locale['449']."</a><br /><br /></div>\n"; // Pimped: make_url
	closetable();
	...
	if ($_GET['error'] < "2") {
		if (!isset($_GET['post_id']) || !isnum($_GET['post_id'])) { redirect(make_url(FORUM."index.php", BASEDIR."forum", "", ".html")); } // Pimped: make_url
		add_to_head("<meta http-equiv='refresh' content='2; url=".make_url(FORUM."viewthread.php?thread_id=".(int)$_GET['thread_id']."&amp;pid=".(int)$_GET['post_id'], BASEDIR."forum-thread-".(int)$_GET['thread_id']."-pid".(int)$_GET['post_id']."-", $seo_data['thread_subject'], ".html")."#post_".(int)$_GET['post_id']."' />\n"); // Pimped: make_url
		...
		echo "<a href='".make_url(FORUM."viewthread.php?thread_id=".(int)$_GET['thread_id']."&amp;pid=".(int)$_GET['post_id'], BASEDIR."forum-thread-".(int)$_GET['thread_id']."-pid".(int)$_GET['post_id']."-", $seo_data['thread_subject'], ".html")."#post_".(int)$_GET['post_id']."'>".$locale['447']."</a> ::\n"; // Pimped: make_url
	} else {
		if (!isset($_GET['thread_id']) || !isnum($_GET['thread_id'])) { redirect(make_url(FORUM."index.php", BASEDIR."forum", "", ".html")); } // Pimped: make_url
		$data = dbarray(dbquery("SELECT post_id FROM ".DB_POSTS." WHERE thread_id='".$_GET['thread_id']."' ORDER BY post_id DESC"));
		add_to_head("<meta http-equiv='refresh' content='4; url=".make_url(FORUM."viewthread.php?thread_id=".(int)$_GET['thread_id']."&amp;pid=".(int)$data['post_id'], BASEDIR."forum-thread-".(int)$_GET['thread_id']."-pid".(int)$data['post_id']."-", $seo_data['thread_subject'], ".html")."#post_".$data['post_id']."' />\n"); // Pimped: make_url
		echo "<a href='".FORUM."viewthread.php?thread_id=".$_GET['thread_id']."&amp;pid=".$data['post_id']."#post_".$data['post_id']."'>".$locale['447']."</a> ::\n"; // constant FORUM added
	}
	echo "<a href='".make_url(FORUM."viewforum.php?forum_id=".$_GET['forum_id'], BASEDIR."forum-".$_GET['forum_id']."-", $seo_data['forum_name'], ".html")."'>".$locale['448']."</a> ::\n"; // Pimped: make_url
	echo "<a href='".make_url(FORUM."index.php", BASEDIR."forum", "", ".html")."'>".$locale['449']."</a><br /><br />\n</div>\n"; // Pimped: make_url
	closetable();
} elseif ($_GET['post'] == "edit") {
	if (!isset($_GET['thread_id']) || !isnum($_GET['thread_id'])) { redirect(make_url(FORUM."index.php", BASEDIR."forum", "", ".html")); } // Pimped: make_url
	add_to_title($locale['global_201'].$locale['409']);
	add_to_head("<meta http-equiv='refresh' content='2; url=".make_url(FORUM."viewthread.php?thread_id=".(int)$_GET['thread_id']."&amp;pid=".(int)$_GET['post_id'], BASEDIR."forum-thread-".(int)$_GET['thread_id']."-pid".(int)$_GET['post_id']."-", $seo_data['thread_subject'], ".html")."#post_".$_GET['post_id']."' />\n"); // Pimped: make_url
	...
	echo "<a href='".make_url(FORUM."viewthread.php?thread_id=".(int)$_GET['thread_id']."&amp;pid=".(int)$_GET['post_id'], BASEDIR."forum-thread-".(int)$_GET['thread_id']."-pid".(int)$_GET['post_id']."-", $seo_data['thread_subject'], ".html")."#post_".$_GET['post_id']."'>".$locale['447']."</a> ::\n"; // Pimped: make_url
	echo "<a href='".make_url(FORUM."viewforum.php?forum_id=".$_GET['forum_id'], BASEDIR."forum-".$_GET['forum_id']."-", $seo_data['forum_name'], ".html")."'>".$locale['448']."</a> ::\n"; // Pimped: make_url
	echo "<a href='".make_url(FORUM."index.php", BASEDIR."forum", "", ".html")."'>".$locale['449']."</a><br /><br />\n</div>\n"; // Pimped: make_url
	closetable();
}
